...
\subsection{Coral polyps and reef formation}
...
From a terrain generation perspective, the \ch{CaCO_3} secretion underlies the elevation of reef structures; light dependence implies depth-based constraints on where reef "height" appears in the heightmap. Moreover, coral-built substrate can be assigned higher resistance values; abstract resistance fields may reflect accumulated skeleton.

\subsection{Coral colony morphologies and zonation}
Coral colonies adopt varied growth forms adapted to local conditions (light, water motion, depth). Major types and their computational implications:

\begin{Itemize}
    \AltTextImage{
        \Item{Massive} (e.g., \textit{Porites}): boulder-like, slow-growing, long-lived, highly damage-resistant. Typically on reef slopes (deeper, more stable). \emph{Terrain effect:} Smooth bulges with high resistance; foundation for surrounding features.
    }{graphics-oceanus_corals-_MG_9848_LuisLamar.jpg}{Large \textit{Porite labota} - Photo credit: Luis Lamar}{fig:intro-Porite}
        %}{Porite-Microatoll.jpg}{A colony of massive \textit{Porites} in a microatoll formation - By R. van Woesik, Y. Golbuu and G. Roff - [1], CC BY-SA 4.0, https://commons.wikimedia.org/w/index.php?curid=54621624}{fig:intro-Porite}

    \AltTextImage{
        \Item{Branching} (e.g., \textit{Acropora}): tree-like, fast-growing, shallow/crest or fore-reef, light-optimized but fragile; regenerate via fragmentation. \emph{Terrain effect:} Fine-scale roughness (high rugosity) in shallow zones; lower resistance patches where branches break.
    }{staghorn-coral-large.jpg}{A field of staghorn coral - Photo credit: NOAA}{fig:intro-Staghorn}

    \AltTextImage{
        \Item{Foliose} (e.g., \textit{Pavona}): leaf-like, layered, sheltering microhabitats; in moderate flow, often on reef flats or back-reef. \emph{Terrain effect:} Overlapping platelets yield intermediate-scale texture; moderate resistance.
    }{Pavona_cactus_colonia_in_situ.jpg}{Foliose corals such as \textit{Pavona Cactus} have a stony skeleton but displays a petal-like structure that provide shelter to the ecosystem - Photo credit: Benzoni, F.}{fig:intro-Pavona}

    \AltTextImage{
        \Item{Table/Plate} (e.g., \textit{Acropora}): flat, expansive in shallow, well-lit areas. \emph{Terrain effect:} Broad, near-horizontal surfaces with edge complexity; influences flow divergence at reef top.
    ...
    \AltTextImage{
        \Item{Encrusting} (e.g., \textit{Cyphastrea}): thin mats over substrate, in deeper or stable areas. \emph{Terrain effect:} Cementing layer that fills gaps, increases cohesion, highest resistance but low vertical relief.
    }{Psammocora.jpg}{Example of a \textit{Psammocora} - Photo credit: Michael Paletta}{fig:intro-Psammocora}
\end{Itemize}
...
Corals act as dynamic ecological agents, engaging in feedback loops with their environment; modeling them requires accounting for spatial diffusion via diverse reproductive modes, competition for resources, and bidirectional environmental influences.
...
% \textbf{Climate stressors:} Elevated temperature and \ch{CO_2} increase bleaching frequency and reduce resilience; pollution and runoff raise nutrient levels, favoring algal blooms.

% \emph{Implication:} Could parameterize coral health or mortality rates in ecosystem simulation; drive scenarios of degradation or recovery.

% \textbf{Sea-level rise response:} Reef vertical accretion may "keep up" (the coral reef stays in the sunlight, just below water), "catch up" (growth slower but eventually matches), or "give up" (cannot match and becomes submerged).

% \emph{Implication:} In procedural generation or dynamic modeling, include constraints on maximum reef elevation based on assumed growth rates vs. sea-level scenarios; can guide long-term morphology or sensitivity analyses.

% % [CORAL COMPETE IN THE ENVIRONMENT]

% Coral and algae both compete for sunlight, substrate, and nutrients. Algae grow faster, especially under elevated nutrients or reduced herbivory, leading to algal overgrowth that smothers coral, flattens reef topography, and reduces habitat complexity and biodiversity. In ecosystem simulations, this behaviour include competitive interactions or state transitions where algae can overtake coral patches under certain field conditions (nutrient/light/herbivore proxies). Terrain impact: algal-dominated zones may be represented by lower-relief substrate without reef elevation.
...
% Structural complexity (rugosity, crevices) offers shelter for juveniles, spawning grounds, and supports diverse species ( about 25\% of marine species in < 0.1\% ocean area). True internal geometry of a reef is not currently completely understood; in our work we abstract it by encoding "presence of complex structure" via functional parameters (e.g., habitat suitability fields, flow resistance modifiers) rather than modeling fine-scale surfaces. 

% This fine-scale properties have influence on the large-scale environment. In fluid or movement simulations, incorporate rugosity/resistance parameters derived from morphology type rather than resolving every crevice. In EFT, these fields influence distribution and behavior of reef-dependent agents.
...
% The erosion process is directly impacted by the small-scale. Reef structures trap or stabilize sediments, protecting mangroves and seagrass beds.
