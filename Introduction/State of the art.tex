...
Procedural generation refers to algorithmically producing content rather than authoring it manually, leveraging rules, mathematical models, or data-driven methods. The primary benefits include reducing storage needs (since content is generated on-the-fly), enabling rapid creation of large or varied content, and supporting both reproducibility and controlled variability. In the context of terrain generation, these advantages allow for the creation of vast landscapes or seascapes with a relatively small parameter set and facilitate interactive pipelines where artists or researchers can iterate quickly without storing every variant.
...
Elevation models are a fundamental approach in terrain representation, widely used in procedural generation due to their simplicity and efficiency. These models define the terrain as a function $h : \R^2 \to \R$, where each point in a 2D plane is mapped to an elevation value. This approach is particularly effective for representing terrains where elevation is the only varying factor, such as hills, valleys, and plateaus, and it is best suited to terrains without complex 3D features such as overhangs or caves. While we visualise elevation models in three dimensions, they are mathematically considered two-dimensional functions. In the domain of terrain generation, we refer to them as 2.5D models, and they are closely related to image processing.
...
Material voxel grids, defined as $\mu: \R^3 \to \material$, can be used in applications where simple occupancy information is not sufficient, but also require internal properties for each voxel from a finite set of possible voxel types such as geological texture or behaviour in a cellular automata point of view. For example, voxel-based games such as \citeProgram{Minecraft2011} utilise material grids to create terrains composed of solid blocks the user can identify easily from their texture (\cref{fig:intro-material-voxels-minecraft}). These voxel grids are a specialisation of layer-based representation whose column height of each material layer is fixed to 1.
