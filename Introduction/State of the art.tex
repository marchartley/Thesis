...
\AltTextImage{
While the term "terrain" describes the physical characteristics of land, it does not include the natural elements that shape an area's identity. Elements such as vegetation, water bodies, and climatic conditions are essential to how we perceive and understand a landscape. Therefore, when discussing procedural generation in virtual environments, "landscape generation" is a more fitting term, as it integrates these natural elements along with the topographical features.
...
Elevation models are a fundamental approach in terrain representation, widely used in procedural generation due to their simplicity and efficiency. These models define the terrain as a function $h : \R^2 \to \R$, where each point in a 2D plane is mapped to an elevation value. This approach is particularly effective for representing terrains where elevation is the only varying factor, such as hills, valleys, and plateaus \comment{Meme moi j'ai pas compris ma dernière phrase...}, and it is best suited to terrains without complex 3D features such as overhangs or caves. While we visualise elevation models in three dimensions, they are mathematically considered two-dimensional functions. In the domain of terrain generation, we refer to them as 2.5D models, and they are closely related to image processing.
...
\AltTextImage{
    ...
    Material voxel grids, defined as $\mu: \R^3 \to \material$, can be used in applications where simple occupancy information is not sufficient, but also require internal properties for each voxel from a finite set of possible voxel types such as geological texture or behaviour in a cellular automata point of view. For example, voxel-based game \citeProgram{Minecraft2011} uses material grids to create terrains composed of solid blocks the user can identify easily from their texture (\cref{fig:intro-material-voxels-minecraft}). These voxel grids are a specialisation of layer-based representation whose column height of each material layer is fixed to 1.
