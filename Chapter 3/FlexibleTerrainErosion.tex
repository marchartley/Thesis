...
\begin{figure}
    ...
    \caption[Particle erosion's pipeline]{Our overall pipeline: our erosion process compute matter displacement of a terrain using an arbitrary representation as long as intersections between particles and the ground can be detected. An optional velocity ﬁeld, provided by the user, guides the particles trajectories. We propose surface alteration methods to apply the erosion to the terrain in a coherent way between possible representations. }
...
\begin{algorithm}[H]
    ...
    \For{$iter \gets 1$ \KwTo $N_{iters}$}{
        $C \gets \emptyset$ \tcp*{Collisions list}
        ...
        \ForEach{$\particle \in \Particles$}{
            \While{$p$ in terrain}{
                ...
                \If{$\particle$ intersects $\Terrain$}{
                    ...
                    add collision data to $C$ \tcp*{particle + terrain properties}\;
...
where $\shearRate = \particleVelocity_{rel}/l$, $\shearStressConstant$ is the shear stress constant (often set to $1$), and $n \in [0,1]$ is the flow behaviour index. Shear-thinning models typically assume $n$ close to $\frac{1}{2}$, which is why we used this value as a constant.  
...
\AltTextImage{
    ...
    \begin{align}
        ...
        \dot{\particleVelocity} = \frac{f(\capacity)\extForce + \dragForce}{\particleMass},
    \end{align}
    with $f(\capacity)$ resulting from \cref{eq:erosion-Richardson}.
...
\begin{figure}[H]
    ...
    \caption[Real world examples of coastal erosion]{(Left) \citeName{Paris2019a} emulate platform coastal erosion, (right) but our method is also able to reproduce the irregular wave-cuts in reef and cliff erosions.}
...
\begin{figure}[H]
    ...
    \caption[Comparison of desertscape erosion with \cite{Paris2019b}]{The algorithm from \citeName{Paris2019b} allows for the generation of desertscapes (top), which we can (at least partially) reproduce with our erosion simulation (bottom). The different effects are achieved by altering the wind direction and strength \cite{Paris2019b}.}
