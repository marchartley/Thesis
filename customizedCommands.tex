...
\DeclareNameFormat{onlyfamily}{%
  % include the prefix (von/de/van…) if present
  \ifblank{\namepartprefix}{}{\namepartprefix\space}%
  \namepartfamily
}

% Cite command that prints just the (first) author's last name in plain text
\DeclareCiteCommand{\citeName}
  {\usebibmacro{prenote}%
  \AtNextCite{\defcounter{maxnames}{1}}% only the first author
  }
  {%
    \usebibmacro{citeindex}%
    \printnames[onlyfamily]{author}%
  }
  {\multicitedelim}
  {\usebibmacro{postnote}}
