...
\begin{Itemize}
    ...
    \Item{Lightweight, analytic field coupling:} A decay-stabilised reaction-diffusion solver for scalar "materials" and analytical deformation of the vector-flow field yield interactive, steady-state updates without heavy fluid or erosion simulation.

    \Item{Multi-scale authoring workflow:} A focus-and-refine pipeline lets users jump from kilometre-scale planning to sub-metre edits while preserving global coherence. We demonstrate this on both coral-reef islands and terrestrial mountain/canyon scenes.
\end{Itemize}
...
\AltTextImage{
Each \gloss{EnvObj} is anchored by a skeleton (a point, curve, or region in the cartographic sense) which captures its footprint while postponing heavy geometry generation.  
\begin{Itemize}
...
We assume a \gloss{SteadyState} regime, guaranteed by a decay rate~$0 < \decay < 1$ that prevents unbounded accumulation during advection-diffusion.
...
\begin{align}
    ...
    \Egradient &= \Agradient \int_{\curve}{\bigl\langle \curve'(s),\nabla^{\perp}\fittingFunc\left(\curve(s)\right)\bigr\rangle ds}
\end{align}
Here $\alpha_{\bullet}$ are user-tunable weights, $\Length$ is an optional target length, and $\nabla^{\perp}f$ denotes the 90°-rotated gradient, encouraging the curve to cut orthogonally across isolines. The minus sign in $\Eexternal$ converts the integrand into an attraction term (high $\fittingFunc$ lowers the energy).
...
\end{align}
where $G$ is a Gaussian kernel centred on the skeleton and $d(\cdot)$ is the planar distance from the skeleton.
...
\begin{Itemize}
  \Item{$\groundedHeight$} grounded data (e.g.\ sea floor patches) defined from the absolute zero level,
