...
Unlike full-fledged ecosystem simulations, which model temporal dynamics (population growth, predator-prey interactions, nutrient cycling, and fluid flows, ...) over successive time steps to study emergent behaviour, our work centers on ecosystem generation, where the goal is to produce a plausible, editable snapshot of a landscape by iteratively placing and culling semantic primitives (\glosses{EnvObj}) that read from and write to steady-state environmental fields; this generation process trades off dynamic fidelity for interactive performance and multi-scale user control, delivering plausible biotic and abiotic scene layouts without the overhead of continuous process simulation.

Building on these observations, the contributions presented in this chapter are:
\begin{Itemize}
    \Item{} \hide{\Gloss{EnvObj} formalism:} a sparse, domain-agnostic symbol set (points, curves, regions) that both read continuous environmental fields and write local modifications back to them, creating a closed feedback loop,
...
A hybrid family, the EFMs \cite{Wu1985}, describes how each individual modifies the state of its surrounding environment. Borrowing field theory from physics, they let organisms deposit or absorb continuous "influence fields", creating explicit feedback loops with the environment.
...
\begin{figure}
    ...
    \caption[Site-based models for ecosystem studies]{Site-based models partition the environment into discrete regions with locally uniform properties. Examples include (left) regular grids, (middle) Voronoi cells, and (right) generalised topological maps \cite{Nelson2012,Lemiere2023}.}
...
\begin{Itemize}
    \Item{} updating dynamically Voronoi diagrams and Delaunay triangulations is computationally more intensive than fixed-grid models, especially in large-scale or highly dynamic ecosystems where individuals frequently appear, disappear, or move.
...
\citeName{Cordonnier2017b} propose a unified landscape simulation coupling hydraulic erosion, weathering, vegetation dynamics, and disturbance events in a time-dependent stochastic framework \cite{Cordonnier2017b}. Their model uses dense layered grids, giving geomorphology and vegetation a shared representation and enabling long-term co-evolution of terrain and plant communities. However, vegetation is encoded as per-cell aggregate properties (density, age, canopy height), which is appropriate for large-scale simulation but prevents manipulation of individual organisms or fine-grained semantic structures. Because our goal is interactive, multi-scale ecosystem generation, we adopt an object-centric representation rather than dense vegetation layers, allowing explicit control over each instance and its contribution to the environment. In this respect, our approach is closer in spirit to individual-based formulations, which explicitly represent organisms or structures as separate entities.

\subsection{Individual-based models}

\begin{figure}
    \autofitgraphics[]{individual-based-modeling-teaser.png}
    \caption[Illustrations of individual-based models used to place trees and modify their visual aspects]{Individual-based models focus attention on each element of the scene, with interaction between neighbouring entities. This perspective allows for more precision on (left) the positioning of each tree \cite{Alsweis2005}, but also on (right) the life cycle and geometry of the individuals \cite{Peytavie2024a}.}
    \label{fig:env-obj-individual-based-models}
\end{figure}

In contrast to aggregated or spatially discrete models, IBMs explicitly represent each organism as a distinct unit \cite{Crooks2017}. IBMs have gained popularity in vegetation modelling because they capture variability in size, spatial positioning, and competitive interactions among individuals. Each tree or plant is modelled independently, with processes such as growth, mortality, and reproduction shaped by local environmental conditions and the presence of neighbours \cite{Chng2013,Peytavie2024a}.

This fine-grained representation enables IBMs to reproduce emergent stand-level dynamics from simple local rules. They are particularly effective for modelling competition for light, nutrients, and space, where outcomes depend on relative size or distance among neighbours \cite{McLane2011,Zhang2020}. To reduce computational costs while maintaining ecological realism, vegetation IBMs often employ distance-based competition methods or stochastic simplifications. Recent advances in parallel computing and GPU simulation have further extended the applicability of IBMs to large-scale virtual ecosystems.
...
The Fixed-Radius Neighbourhood (FRN) method is the first appearance of a distance-based method. Considering an application to forest ecosystems, each tree has a radius inside which we consider that there is shade and competition for nutrients. If two trees' radii intersect, a competition allows the stronger tree to survive while the weaker one dies \cite{Deussen1998}. %(\cref{fig:env-obj-individual-based-models}, left).

\AltTextImage{
    In graphics, FRN-based exclusion is realised by Poisson-disk sampling (\cref{fig:env-obj-poisson-example}) or blue noise approximations \cite{Deussen2000} to generate entities as points that do not fall within another entity's radius. The algorithm can be accelerated using a regular grid structure as proposed by \citeName{Bridson2007b} \cite{Bridson2007b}: each cell of the grid has a width of $\frac{\radius}{\sqrt{n}}$ with $n$ the number of dimensions (usually $n = 2$). Each cell stores at most one sample, limiting neighbour checks to adjacent cells. Each point added to the grid recursively instantiates new random samples in a spherical annulus between radius $\radius$ and $2\radius$. Variants of this method guarantee maximal coverage \cite{Ebeida2011} or propose a GPU parallelisation \cite{Wei2008}. However, in the presence of at least two species with different radii, the circle packing problem becomes more complicated and requires a larger number of collision checks, while still relying on the spatial indexing method from Bridson's algorithm.

}{Poisson_disk_sampling.png}{Poisson Disk Sampling iteratively adds points in space and rejects a candidate if it is within radius $\radius$ of any existing point. }{fig:env-obj-poisson-example}{Poisson Disk Sampling}

\AltTextImage{
    \subsubsection{Zone of Influence}
    The Zone of Influence (ZOI) model, used in \cref{fig:env-obj-individual-based-models} (left) \cite{Alsweis2005,Alsweis2006} is an ecological method designed to quantify competition among trees by modelling their interactions as gradual rather than binary processes \cite{Czaran1998}. In contrast to a simple binary approache proposed with FRN, the ZOI model assigns each tree a circular influence zone, typically defined based on measurable attributes such as Diameter at Breast Height (DBH), crown dimensions, or species-specific ecological characteristics.
...
\end{align*}
where $\Area_{\text{overlap}, ij}$ is the area of overlap between the radii of trees $i$ and $j$, and $\Area_{\text{ZOI}, i}$ is the total area of the influence zone of tree $i$. This intersection area is computed directly with
\begin{align*}
...
The biological consequences of competition quantified by ZOI, such as reduced growth or increased mortality risk, are represented through growth-response equations. For instance, tree growth under competition might be modelled as \cite{Das2012,Uriarte2004}:
\begin{align*}
...
}{FON-Peters2020.png}{The FON model proposes a smoother version than the ZOI, where the intersection between two radii also accounts for the distance from the tree trunk \cite{Peters2020}.}{fig:env-obj-fon-scheme}{Field of Neighbouthood competition}
...
\AltTextImage{
    ...
    \begin{align*}
        ...
        \end{dcases}
    \end{align*}
    with RBH the radius at breast height ($\text{RBH} = \frac{1}{2} \text{DBH}$) (\cref{fig:env-obj-dbh}), $\Radius$ the radius of influence of the tree (usually proposed as $\Radius = a \sqrt{\text{RBH}}$ with a scaling parameter $a$), and $C$ an attenuation coefficient, defined for each species.
}{DBH-measure-2.png}{DBH (and RBH) is measured at 1.3 m above ground level.}{fig:env-obj-dbh}{Diameter and Radius at Breast Height}
...
\AltTextImage{
    ...
    \end{align*}
}{FON-scheme-Berger2000.png}{The evaluation of the FON value at any point is done by accumulating all individual fields \cite{Berger2000}.}{fig:env-obj-fon-compute-scheme}{Evaluation of the FON value at any point by accumulating all individual fields}
...
\AltTextImage{
    By designing non-monotonic field functions around trees, emergent behaviours can appear. \citeName{Lane2002} proposed to apply what they call a deformation kernel on the distance function, resulting in a larger variety of patterns, such as the clustering of bushes and the competition of larger trees (\cref{fig:env-obj-kernels-Lane2002}, top left) \cite{Lane2002}. Building on this idea, defining deformation kernels for each pair of species (\cref{fig:env-obj-kernels-Lane2002}, bottom right, presents a kernel matrix for two species) allows the system to include ecological interactions between different species in the terrain. As a result, the simulation can introduce intra-species clumping while maintaining inter-species segregation.
}{kernel-matrices-Lane2002-s.png}{Deforming neighbourhood kernels models species behaviours and interactions. Left: different kernels (top) yield distinct layouts (middle), that can be encoded in a inter-species matrix $M$ (bottom); Right: resulting distribution \cite{Lane2002}.}{fig:env-obj-kernels-Lane2002}{\cite{Lane2002}'s deformation kernel for inter-species vegetation distribution}
% \AltTextImage{
%     By designing non-monotonic field functions around trees, emergent behaviours can appear. \citeName{Lane2002} proposed to apply what they call a deformation kernel on the distance function, resulting in a larger variety of patterns, such as the clustering of bushes and the competition of larger trees (\cref{fig:env-obj-kernels-Lane2002}, top left) \cite{Lane2002}. Building on this idea, defining deformation kernels for each pair of species (\cref{fig:env-obj-kernels-Lane2002}, top right, presents a kernel matrix for two species) allows the system to include ecological interactions between different species in the terrain. As a result, the simulation can introduce intra-species clumping while maintaining inter-species segregation.
% }{kernel-matrices-Lane2002.png}{The deformation of neighbourhood kernels enables modelling of different behaviours for species and their interactions. Top-left: different kernels generate different layouts; Top-right: a matrix $M$ encodes a distance kernel for each pair of species; Bottom: using a single matrix provides a way to control the interaction of two species in a scene \cite{Lane2002}.}{fig:env-obj-kernels-Lane2002}{\cite{Lane2002}'s deformation kernel for inter-species vegetation distribution}
\AltTextImage{
    Alternatively, learning the spatial distribution of distances between individuals from examples can be used as a statistical metric to introduce interactive tools for ecosystem generation \cite{Emilien2015a} (\cref{fig:env-obj-worldbrush-Emilien2015}). In this work, a user-provided patch serves as an example: the system extracts descriptors such as histograms of inter- and intra-species distances, or correlations with environmental variables such as slope, and reproduces these distributions in other regions. Unlike kernel-based models, which require explicit design of influence functions, this approach learns statistical descriptors directly from data. Introducing a metric based on distances between disks rather than point locations makes it possible to capture additional spatial and topological relationships between individuals. This includes behaviours such as a species tending to grow beneath, along, or away from another species' influence zone \cite{EcormierNocca2019}. These Pairwise Correlation Functions (PCF) can be extracted either from a user-designed example scene or from the output of a complex, biologically driven ecosystem simulator \cite{Gain2017}, enabling the replication of these spatial patterns without requiring full ecological simulation.
}{worldbrush-Emilien2015.png}{Using statistical distribution from an input terrain, a different terrain with similar distributions is synthesised \cite{Emilien2015a}. Two examples are given (delimited by blue rectangular boundaries): uniform distribution (top) and clustered distribution (bottom). Differences in distribution arise from the voids in the regions of interest.}{fig:env-obj-worldbrush-Emilien2015}{\cite{Emilien2015a}'s ecosystem simulation by statistic analysis and distribution}

These methods are not limited to vegetation but generalises to any scene elements (e.g., houses, rocks, roads), making it highly flexible for procedural content generation. From an ecological standpoint, it resonates with point-process modelling, where distributions of distances or clustering statistics are fitted to observed spatial patterns \cite{BenSaid2021,Baddeley2010}. For computer graphics, the strengths of this method are interactivity and scalability: complex, realistic layouts can be synthesised quickly without costly simulation, while retaining ecological plausibility through statistical consistency.
...
\begin{figure}
    ...
    \caption[Ecological Field Model with trees, bushes and flowers]{EFMs represent the environment as a mathematical function where environmental properties such as humidity, shade, and nutrients are seen as distinct fields (from left to right respectively) created by the presence of trees, bushes, and flowers \cite{Wu1985}.}
...
\begin{align}
    \eqlabel{eq:env-obj-eco-field-computation}{Ecological field evaluation expression}
...
\AltTextImage{
    Using the field values, the system instantiates small details such as leaves by sampling the space with candidate "anchor positions" (\cref{fig:env-obj-teaser-grosbellet2016}, right). If the field values satisfy certain conditions at the anchor position $\p_i$, the small-scale objects are instantiated at this position (for example, for leaves: $\alpha(\p_i) l(\p_i) < d_i$ using the occlusion field $\alpha$, the leaves deposition field $l$ and the distance of the anchor position to the depositing surface $d_i$ as shown in \cref{fig:env-obj-leaves-instances}). This method was extended to modify the geometry or texture of the object based on the field values (in \cref{fig:env-obj-leaves-instances}, the heat field and the humidity field), to manually move or rotate each of the objects, or even integrate user-defined density fields to populate entangled details on the ground \cite{Guerin2016}.
}{EnvObjs-leaves-instances-Grosbellet2016.png}{Leaves are instanced if the field values are fitted for leaves presence, and the geometry and texture are altered depending on the environment \cite{Grosbellet2016}. }{fig:env-obj-leaves-instances}{Instantiation of leaves in \cite{Grosbellet2016}}

\AltTextImage{
    On the other hand, the geometry of large objects is altered using the scalar fields. The authors present a simulation of snow deposition on the objects by displacing the vertices $i$ in the direction of the original surface normals $\vec n_i$ by an amount depending on the environment (\cref{fig:env-obj-snow-Grosbellet2016}):
    \begin{align*}
...
}{EnvObjs-snow-Grosbellet2016.png}{Snow deposition is a displacement of vertices along the surface normals of the mesh (left), modulated by the value of fields such as occlusion (middle) and heat (right) \cite{Grosbellet2016}. }{fig:env-obj-snow-Grosbellet2016}{Snow deposition and the effects of occlusion and heat fields in \cite{Grosbellet2016}}

\AltTextImage{
    In addition to the EFM framework, environmental effects from environmental objects are not limited to point sources as the scalar functions may use a spline primitive or be extended to volume primitives (\cref{fig:env-obj-heat-field-effect}). Translated to the ecological domain, this methodology extends the EFM as the inclusion of water bodies, essential for realistic vegetation simulation, could be taken into account by considering environmental effects from rivers and lakes which are often modelled as splines and regions in 3D modelling \cite{Emilien2015,Genevaux2013,Genevaux2015}.
}{env-obj-fields-heat-Grosbellet2016.png}{The heat field produced by a pipe uses a distance function from a line instead of a single point in space \cite{Grosbellet2016}.}{fig:env-obj-heat-field-effect}{Heat field from a pipe}
...
\begin{align}
    \eqlabel{eq:reaction-diffusion-classic}{Classical reaction-diffusion formulation}
...
Diffusion models the random spread of material and follows Fick's law, which states that the diffusive flux is proportional to the gradient of the field:
\begin{align*}
    % \eqlabel{eq:reaction-diffusion-flicks-law}{Flick's law}
    \text{Flux} = -D \nabla u.
\end{align*}

Because diffusion conserves mass locally, the concentration must also satisfy the continuity equation:
\begin{align*}
    % \eqlabel{eq:reaction-diffusion-conservation}{Conservation law}
    \frac{\partial u}{\partial t} = -\nabla \cdot (\text{Flux}).
\end{align*}

Combining the two relations directly gives the diffusion term in \cref{eq:reaction-diffusion-classic}:
\begin{align}
    \eqlabel{eq:reaction-diffusion-diffusion-term}{Diffusive flux}
    \frac{\partial u}{\partial t} = -\nabla\cdot(-D\nabla u) = D\nabla^2 u.
\end{align}
...
\begin{align}
    \eqlabel{eq:reaction-diffusion-advection}{Advection-Reaction-Diffusion (ARD) equation}
    \frac{\partial u}{\partial t} + \field{v}\cdot \nabla u = D \nabla^2 u + R(u).
\end{align}
Unlike diffusion's random spread, advection shifts material coherently along streamlines, aligning with our processing of water currents as vector fields \cite{Burger2020,Ogden2017}.  
In graphics, this formulation underpins fluid solvers such as Stam's "stable fluids" \cite{Stam1999}, where advection steps move smoke, fire, or water density across a grid. GPU-accelerated advection is also widely used in real-time effects such as smoke trails or rivers, making it a natural tool to simulate ecological transport processes, which is presented in depth in \cref{chap:erosion}'s state of the art.

\subsubsubsection{Decay and steady state}
To avoid indefinite accumulation, a decay term $-\decay u$ is added:
\begin{align}
    \eqlabel{eq:reaction-diffusion-advection-decay}{ARD with linear decay}
...
\begin{align}
    \eqlabel{eq:reaction-diffusion-diffusion-green-point-source}{Green's function for a single point source in diffusion-decay setup}
...
For extended sources such as curves $\curve$ or regions $\Omega$, the solution is the convolution of the point-source Green's function with the source geometry:
\begin{align}
    \eqlabel{eq:reaction-diffusion-diffusion-green-curves}{Extended-source solution with convolution}
    u(x,t) &= \int_\curve G(x-y,t)\,dy, \\
...
\begin{figure}[H]
    \autofitgraphics{new-pipeline.pdf}
    \caption[\cref{chap:semantic-representation}'s pipeline]{Overview of the pipeline of our iterative generation process: at each iteration, (1) the environment instantiate a new \gloss{EnvObj}, (2) \glosses{EnvObj} evaluate their fitness with respect to user modifications then (3) update the environment (\cref{sec:env-obj-generation-process}) until (4) stabilisation with user-defined geomorphological events (\cref{sec:env-obj-events}). When the resulting scene suits the user, the sparse representation of generated \glosses{EnvObj} is returned (\cref{sec:env-obj-output}) and can be instantiated and rendered. Ecologist knowledge (green) is injected through material and object properties, while user control (yellow) appear on instantiated objects, environmental attributes, and stopping conditions. }
    \label{fig:env-obj-pipeline}
\end{figure}
% \begin{figure}[H]
%     \autofitgraphics{pipeline.pdf}
%     \caption[\cref{chap:semantic-representation}'s pipeline]{Overview of the pipeline of our method. Three different inputs are required at initialisation (left): a base terrain and a water level optionally given by the user for each generated scene, a set of properties for the \glosses{EnvMat} available, and the properties and generation rules of \glosses{EnvObj} available, established with experts (\cref{sec:env-obj-initialisation}). The generation process (middle) iteratively proceed in three steps: instantiating new \glosses{EnvObj}, evaluating the fitness of each entity with respect to user modifications, and updating the environment (\cref{sec:env-obj-generation-process}) until stabilisation with user-defined geomorphological events (\cref{sec:env-obj-events}). When the resulting scene suits the user, the sparse representation of generated \glosses{EnvObj} is returned (\cref{sec:env-obj-output}) and can be instantiated and rendered (right). }
%     \label{fig:env-obj-pipeline}
% \end{figure}

\subsection{Overview}

Our method generates plausible, editable environments by coupling discrete semantic primitives with continuous environmental fields. As illustrated in \cref{fig:env-obj-pipeline} and summarised in \cref{alg:env-obj-pipeline}, the system forms a closed feedback loop in which sparse objects (\glosses{EnvObj}) interact with environmental quantities (\glosses{EnvVal}) through local modifiers (\glosses{EnvModif}). This captures how terrain features both depend on and reshape their surroundings.

The framework rests on three complementary abstractions (introduced in \cref{sec:env-obj-core-concepts}): 
\begin{Itemize}
    \Item{} \glosses{EnvVal} describe the environment as continuous scalar and vector fields, 
    \Item{} \glosses{EnvObj} are sparse, scale-independent primitives with a skeleton and per-class rules, 
    \Item{} and \glosses{EnvModif} mediate their mutual influence by translating object presence into local field updates.
\end{Itemize}

Generation proceeds iteratively: starting from an initial configuration of terrain, water level, materials, and an object catalogue, the system alternates between object instantiation and environment updates (\cref{sec:env-obj-generation-process}). Users can intervene at any time by editing objects or triggering \glosses{GeoEvent} (\cref{sec:env-obj-interaction}). The process converges to a steady state and yields a sparse semantic scene suitable for meshing or rendering (\cref{sec:env-obj-output}).

% This section introduces the overall architecture of our method before detailing its components and execution. At a high level (\cref{fig:env-obj-pipeline}, \cref{alg:env-obj-pipeline}), we generate plausible, editable environments by instantiating sparse semantic primitives (\glosses{EnvObj}) that \emph{read from} and \emph{write to} continuous environmental fields (\glosses{EnvVal}), iterating to a steady state while allowing multi-scale user control.

% \paragraph{Core components at a glance.}
% \begin{Itemize}
%   \Item \textbf{\glosses{EnvVal}}: continuous fields composing the environment $\environment=(\height,\;\Water,\;\Wlevel,\;\material)$. Here, $\height$ is the signed height (relative to $\Wlevel$), $\Water$ is the 2D water-current vector field, $\Wlevel$ the water level, and $\material$ a set of per-point resource stocks (e.g., sand, limestone, shade).
%   \Item \textbf{\glosses{EnvObj}}: sparse, scale-agnostic primitives (points/curves/regions) that carry a semantic class and a lightweight \emph{skeleton}. Each object queries $\environment$ via a \gloss{FitnessFunc} to assess viability and a \gloss{FittingFunc} to optimise its skeleton (e.g., Snake optimisation for curves/regions).
%   \Item \textbf{\glosses{EnvModif}}: local write-backs from objects to fields, $\environmentModif=(\heightModif,\;\waterModif,\;\materialModif)$, which update coarse elevation, deform the flow, and deposit/absorb \glosses{EnvMat}.
% \end{Itemize}

% \paragraph{Generation process (what happens each step).}
% \begin{Itemize}
%   \Item \textbf{Initialisation.} From a base height field $h$ and water level $\Wlevel$, we set $\height=h-\Wlevel$, initialise $\material$, and assemble a terrain-aware background flow $\Wsimu$; the global flow is $\Water=\Wuser+\Wsimu+\Wobj$.
%   \Item \textbf{Iterative loop.} Each iteration proposes new \glosses{EnvObj} at high-fitness locations, refines their skeletons, and applies their \glosses{EnvModif}. Scalars in $\material$ evolve under an advection–diffusion–reaction update with decay (steady-state stabilisation), height is aggregated analytically from object groups, and $\Water$ combines user strokes $\Wuser$, analytic terrain flow $\Wsimu$, and object-induced deformations $\Wobj$ (Kelvinlets). Users may edit objects or trigger \glosses{GeoEvent} (water-level change, subsidence/uplift, storms), which analytically modify the fields.
%   \Item \textbf{Termination.} The loop stops when a target is reached (e.g., object counts), a maximum iteration is met, or the user approves a steady snapshot.
% \end{Itemize}

% \paragraph{Output.}
% The method returns a sparse semantic description of the scene: \glosses{EnvObj} with their 2D skeletons, classes, and modifier parameters. Geometry, meshing, and rendering are deferred to downstream tools; the exported representation preserves multi-scale coherence and supports interactive editing.
...
\begin{algorithm}%[H]
    \DontPrintSemicolon
    \caption{\glosses{EnvObj} generation pipeline.}
    \label{alg:env-obj-pipeline}
    \KwIn{Base height field $h$, water level $\Wlevel$, user flow $\Wuser$, catalogue $\availableObjects$, target multiset $\targetObjects$, material params for $\material$}
    \KwOut{Sparse set of instantiated \glosses{EnvObj}}
    \BlankLine
    % --- Initialisation ---
    \textbf{Initialisation:}\;
    $\height \gets h - \Wlevel$\;
    Initialise $\material$ scalar fields (zeros or expert priors)
    $\Wsimu \gets \text{terrain-aware flow from } \height$
    $\Water \gets \Wuser + \Wsimu$ 
    $\environment \gets (\height,\;\Water,\;\Wlevel,\;\material)$ \;
    $\objects \gets \varnothing$ \;

    % --- Main loop ---
    \textbf{Generation loop:}\;
    \While{\text{not stop (max iters / $\targetObjects$ reached / user approval)}}{
        % Instantiation
        \tcp{(1) Instantiate new objects }
        \ForEach{$\text{class} \in \availableObjects$}{
            sample position: $\p_s \gets \arg\max_{\p_k}\;\fitnessFunc_{\text{class}}(\p_k,\environment)$\;
            \If{$\fitnessFunc_{\text{class}}(\p_s,\environment) > threshold_{\text{class}}$}{
            compute object's \gloss{Skeleton}: $\gloss{Skeleton}_o \gets \fittingFunc_{\text{class}}(\environment)$\;
            add new object $o$ to $\objects$\;
            }
        }

        % Environment update
        \tcp{(2) Environment update from \glosses{EnvModif}}
        $\height \gets \text{recompute from all objects height modifiers } \heightModif_o$\;
        $\Wsimu \gets \text{recompute from } \height$\;
        $\Wobj \gets \sum_{o \in \objects}\lambda_o\,\tensor{u}_o$\;
        $\Water \gets \Wuser + \Wsimu + \Wobj$\;
        $\material \gets \text{advection-reaction-diffusion}$\;
        $\environment \gets (\height,\;\Water,\;\Wlevel,\;\material)$\;
        apply direct user edits to \gloss{Skeleton}\;
        apply \glosses{GeoEvent} analytically on $\environment$\;
        $\objects \gets \text{remove unfit objects}$\;
    }
    \Return{$\objects$} \tcp*{sparse \glosses{EnvObj}}
\end{algorithm}
...
\label{sec:env-obj-core-concepts}

Our approach extends the concept of \glosses{EnvObj}, simplified, scale-agnostic terrain features that interact with their surroundings to simulate complex ecosystem dynamics. These \glosses{EnvObj}, which represent natural features (corals, algae, islands, rocks, ...) influence and are influenced by environmental fields (temperature, humidity, elevation, ...). 

In this section, we present the different related concepts that underpins our method.
We first introduce the continuous environmental fields (\glosses{EnvVal}), then the discrete actors that perceive and affect these fields (\glosses{EnvObj}), before detailing how they modify the fields (\glosses{EnvModif}) and how the full pipeline orchestrates their interaction.

\subsubsection{\Glosses{EnvVal}}
\label{sec:env-obj-communication}

In the field of geography and for EFMs, a "field" is a spatially continuous variable defined at every point of the domain, encompassing scalar fields (elevation, temperature) and vector fields (wind, current). In the following, we name every such quantity an \gloss{EnvVal}.
...
\subsubsection{\Glosses{EnvObj}}
\label{sec:env-obj-environmental-objects}

% \begin{figure}
%     \autofitgraphics{assets-demo.pdf}
%     \caption[Different \glosses{EnvObj}'s visuals]{\Glosses{EnvObj} can be visualised by 3D triangular meshes. From left to right: rocks, trees, corals, arches.}
...
\AltTextImage{
Each \gloss{EnvObj} is anchored by a skeleton (a point, curve, or region in the cartographic sense) which captures its footprint while postponing heavy geometry generation (\cref{fig:env-objs_example-scheme1}). Our objects are:
\begin{Itemize}
...
\end{Itemize}
}{Env-obj-scheme1.png}{Environmental objects, from 2D abstract geometric shapes (top) to a possible 3D geometry (bottom).}{fig:env-objs_example-scheme1}{Illustration of the abstract \glosses{EnvObj} geometry and the 3D translation}
...
\subsubsection{\Glosses{EnvModif}}
...
% \begin{Itemize}
%     \Item{} \textbf{initialisation:} sets up the foundational terrain elements and \glosses{EnvObj} definitions; 
%     \Item{} \textbf{iterative generation process:} instantiates \glosses{EnvObj} and updates their surrounding environment; 
%     \Item{} \textbf{output stage:} that yields a sparse representation of the scene's features. 
% \end{Itemize}
...
\label{sec:env-obj-initialisation}

We initialise the terrain with a height field $h$ and a water level $\Wlevel$. As a large number of underwater \glosses{EnvObj} depends on altitude or depth, we  use a shorthand notations $\height = h-\Wlevel$ (signed height above water) and $\depth=-\height$ (signed depth below water). The height field provides variation in altitude, which influence the generation process of the scene.
...
\label{sec:env-obj-generation-process}

Once initialisation is complete, we enter an iterative generation phase in which objects and environment co-evolve until a steady state is reached. Each iteration alternates between two main stages: 
\begin{Itemize}
    \Item{} \textbf{object instantiation and shaping}, where new \glosses{EnvObj} are proposed, evaluated, and fitted to their surroundings and
    \Item{} \textbf{environment update}, where these objects modify the environmental fields through their \glosses{EnvModif}.
\end{Itemize}

The iterative loop continues until user-defined termination criteria are met such as a fixed number of iterations, the completion of a target object set $\targetObjects$, or explicit user approval.  
During this process, users may intervene at any time to steer the generation, either by editing individual \glosses{EnvObj} interactively or by triggering large-scale \glosses{GeoEvent} that globally affect the environmental fields.  
...
\subsubsection{Placement of \glosses{EnvObj} in an environment}
...
\paragraph{\Gloss{FittingFunc}}
\label{sec:env-obj-fitting-func}

The candidate object is first seeded at its optimal position $\p_s = \arg \max_\p \fitnessFunc(\p)$. This seed acts as the initial guess for the subsequent shape optimisation for the \gloss{FittingFunc} $\fittingFunc(\environment): \R^2 \to \R$ described below. Moreover, this two-step optimisation allows a coarse approximation of the global maximum of the \gloss{FitnessFunc} with few samples, reducing its computational demand, as the \gloss{FittingFunc} optimisation is focused on a single seed.

\subsubsubsection{Point-based \gloss{Skeleton}}
...
\subsubsubsection{Curve-based \gloss{Skeleton}}

The skeleton of a curve-based \gloss{EnvObj} is best fitting shape in the environment it is added to.
Using an Active-Contour ("snake") formulation \cite{Kass1988}, we seek a parametric curve $\curve$ that minimises the following composite energy:
\begin{align}
    \eqlabel{eq:env-obj-snake}{Modified Snake energy}
    \energy(\curve) = \Einternal + \Eexternal + \Eshape + \Egradient,
\end{align}
% with
% \begin{align}
%     \eqlabel{eq:env-obj-snake-energy-equation-internal}{Modified Snake internal component}
    % \Einternal &= \Ainternal \left( \int_{\curve}{ \norm{\curve'(s)}^2 \, ds} + \int_{\curve}{ \norm{\curve''(s)}^2 \, ds} \right), \\
    % \eqlabel{eq:env-obj-snake-energy-equation-external}{Modified Snake external component}
%     \Eexternal &= -\Aexternal \int_{\curve}{ \fittingFunc(\curve(s)) \, ds }, \\
%     \eqlabel{eq:env-obj-snake-energy-equation-shape}{Modified Snake shape component}
%     \Eshape &= \Ashape \left( \Length - \int_{\curve}{ \, ds } \right)^2, \\
%     \eqlabel{eq:env-obj-snake-energy-equation-gradient}{Modified Snake gradient-alignment component}
...
\AltTextImage{
    ...
    \begin{align}
        ...
        \Einternal &= \Ainternal \left( \int_{\curve}{ \norm{\curve'(s)}^2 \, ds} + \int_{\curve}{ \norm{\curve''(s)}^2 \, ds} \right).
    \end{align}
}{snake_algorithm_internal.pdf}{Internal energy minimisation for one vertex is done by reducing the difference of length with the next and with the previous vertices (tension, left), while reducing the curvature at this point (rigidity, right).}{fig:env-obj-snake-internal}{Snake algorithm - Internal energy}

In \cref{fig:env-obj-snake-internal} we see the two different components of this energy minimisation: averaging the vector formed by the previous segment with the vector formed by the next segment, we move the vertex to a position for which the two new segments are equal in length. On the other hand, translating the vertex towards its projection minimises the curvature at this vertex.

\AltTextImage{
    \Item{} External energy: \\
...
}{snake_algorithm_external.pdf}{The central vertex external energy is optimised by translating in the same direction as the function's gradient $\nabla \fitnessFunc$.}{fig:env-obj-snake-external}{Snake algorithm - External energy}
...
}{snake_algorithm_shape.pdf}{The shape energy is minimised when the arc length of the curve $\curve$ is equal to a parameter $\Length$, obtainable by moving towards or in opposition to its neighbours.}{fig:env-obj-snake-shape}{Snake algorithm - Shape energy}

\AltTextImage{
    \Item{} Gradient energy: \\
    In our application, having information about the orientation of \glosses{EnvObj} may be essential, such as guiding a river in the direction of the slope. For this purpose, we introduced a gradient energy component in the formulation. The equation imposes that the direction of the curve at any point should be directed towards the gradient of the scalar field (\cref{fig:env-obj-snake-gradient}): 
    ...
    }{snake_algorithm_gradient.pdf}{The gradient energy is minimised when the curve crosses perpendicularly the isolines of the function. Black: the initial curve, Blue: a possible curve crossing the gradient obtained by displacing the vertices. % The scalar field is stylised to display isolines that the optimal curve has to cross.
}{fig:env-obj-snake-gradient}{Snake algorithm - Gradient energy}
...
\begin{figure}
    \labelledautofitgraphics[]{$\Agradient > 0$}{snake1.png, snake2.png, snake3.png}
    \rule{\textwidth}{0.5pt}
    \labelledautofitgraphics[]{$\Agradient = 0$}{snake1-noGrad.png, snake2-noGrad.png, snake3-noGrad.png}
    \rule{\textwidth}{0.5pt}
    \labelledautofitgraphics[]{$\Agradient < 0$}{snake1-negGrad.png, snake2-negGrad.png, snake3-negGrad.png}
    \caption[Three examples of Snake optimisations with our gradient energy component]{Three examples of Snake optimisation from an initial curve (orange line). One vertex of the curve is fixed. Top row has a strong gradient energy coefficient $\Agradient > 0$, middle row has no gradient energy coefficient $\Agradient=0$, and bottom row has negative gradient energy coefficent $\Agradient < 0$. }
    \label{fig:env-obj-snake-gradient-example}
\end{figure}

\subsubsubsection{Region-based \gloss{Skeleton}}
...
\begin{align}
    \eqlabel{eq:env-obj-snake-external-on-shapes}{Chan-Vese region snake external energy}
...
Finally, the shape constraint energy $\Eshape$ targets an area $\Area$:
\begin{align*}
    \Eshape = \Ashape \left( \Area - \int_{\domain}{1 \, ds } \right)^2.
\end{align*}
...
At every global iteration, each object re-evaluates $\fitnessFunc(\curve, \environment) < T_{\text{fit}}$ with $T_{\text{fit}}$ a threshold for which the object is removed, fixed for each object class; otherwise we take advantage of the iterative nature of the Snake algorithm and improve the shape of the \gloss{Skeleton} by optimising their \gloss{FittingFunc} again. If the user fixed the position of vertices, we simply set $\nabla \energy = \vec{0}$ at the given vertices. %, multiple global iterations after their instantiation. As the number of \glosses{EnvObj} becomes large, readjusting all \glosses{Skeleton} becomes time-consuming, so we can use this process for the $n$ most recent \glosses{EnvObj} in the scene.

% ---------- Environment modifiers -------------------------------------------
% \subsubsection{\Glosses{EnvModif}}
\subsubsection{\Gloss{EnvMat} modifiers}
...
\begin{align}
    \eqlabel{eq:env-obj-environment-update}{Environment evaluation from \glosses{EnvModif}}
...
\begin{align}
    \eqlabel{eq:env-obj-deposition-absorption-per-object}{Per-object absorption-deposition}
...
\begin{align}
    \eqlabel{eq:env-obj-material-advection}{Material transport velocity}
    \warp(\p, t) = \mass_\oneMaterial \nabla \height(\p, t) + \velFactor_\oneMaterial \Water(\p, t).
\end{align}

The \glosses{EnvMat} are also dispersed at a diffusion rate $\diffusion_\oneMaterial$, for which we use the advection-diffusion-reaction equation to evaluate the distribution over time $t$:
\begin{align}
    \eqlabel{eq:material-displacement-equation}{Advection-Reaction-Diffusion for \glosses{EnvMat}}
...
\begin{align}
    \eqlabel{eq:material-displacement-Euler}{Forward-Euler iteration for ARD}
...
The linear decay $\decay_\oneMaterial$ prevents indefinite mass build-up and drives the system toward a bounded equilibrium, following the practice of EFMs \cite{Seidl2012}. \cref{fig:env-obj-stability-examples} shows the stability of a diffusion simulation over 200s with and without decay, with and without a flow field. A steady state is visible as the MAE (Mean Absolute Error, i.e. discrete approximation of $L^1$-norm) from one time step to another drops at $t=50s$ in the case of a small decay, while the MAE diverges in the case without decay. The implementation of this process is straightforward, as summarised by the concise pseudo-code in \cref{alg:env-obj-advection-reaction-diffusion}, which integrates advection, diffusion, decay, and source terms in an iterative loop.
...
\begin{algorithm}%[H]
    \DontPrintSemicolon
    \caption{Advection-diffusion-reaction for a single \gloss{EnvMat}.}
    \label{alg:env-obj-advection-reaction-diffusion}
    \KwIn{$\concentration(\cdot)$, $\Water$, $\height$, params $(\mass,\velFactor,\diffusion,\decay)$, sources and sinks $S(\cdot)$, step size $dt$}
    \KwOut{Steady state \gloss{EnvMat}}
    \BlankLine
    $\warp(\p) \gets \mass \nabla \height(\p) + \velFactor \Water(\p)$ for all $\p$\;
    $\hat{\concentration} \gets \concentration$\;
    \DoWhile{$\frac{1}{|\Omega|}\sum_{\p} |\hat{\concentration}(\p)-\concentration(\p)| > \tau$ \tcp*{MAE < $\tau$}}{
        $\concentration \gets \hat{\concentration}$\;
        \ForEach{$\p$}{
            $\hat{\concentration}(\p) \gets \max\{0,\; \concentration(\p) + dt\,[\diffusion \nabla^2 \concentration
            - \decay \concentration - \warp\cdot\nabla \concentration + S(\p)]\}$\;
        }
    }
...
\begin{figure}
    ...
    \caption[\gloss{EnvMat} deposition with multiple \glosses{EnvObj}]{(Left) A single island deposits sand over its region, which is displaced by the water flow. (Centre) With two adjacent islands, we see an accumulation of sand (green) in the valley they form, as we would see in nature. (Right) This phenomenon is directly due to the cancelling of their respective water flow influence, producing a region with low currents, allowing sand to settle, without any need for complex computation.}
    \label{fig:env-obj-double-island-sand-deposition}
\end{figure}

\begin{figure}
    \autofitgraphics[]{2025-06-20__09-50-22_render_top_view-with-shade-with-flow-with-objects.png, 2025-06-20__09-50-22_render_top_view-with-shade.png}
    \autofitcaptions[]{Scene with tree meshes and flow field, Without meshes and flow field}
    \caption[Representing shade as an \gloss{EnvMat}]{While we do consider shade (red) as being a \gloss{EnvMat} in the same manner as sand or pebbles, setting the influence of water currents and slope to null makes it independent of the environment, staying at the position of the yellow \gloss{EnvObj} that generates it.}
...
\begin{figure}[H]
    ...
    \caption[Effect of decay on advection-diffusion-reaction simulations]{With the addition of a decay term in the advection-diffusion-reaction equation, the spread of the \glosses{EnvMat} tends towards stability after a few iterations (right), whereas the total mass would otherwise increase indefinitely (left). Concentration values are clamped to 1 for visualisation. This behaviour also holds in the presence of a flow field (bottom), here with bounded diagonal vector field oriented along $x=y$. Moreover, decay reduces boundary-related instabilities; bottom left uses a zero-gradient boundary condition, which leads to unstability from the borders.}
    \label{fig:env-obj-stability-examples}
\end{figure}

% \subsubsubsubsection{Approximation of distances from \glosses{EnvMat}}
\subsubsubsection{Approximation of distances from \glosses{EnvMat}.}

The use of \glosses{EnvMat} provides an efficient, on-demand approximation of the distance to the closest instance of an object type $\objects$, with computational complexity linear in the number of objects in the scene. For each object type (e.g., coral, rock, reef, island, ...), we maintain a separate \gloss{EnvMat} field $\material_\text{object}$ that evolves under diffusion-reaction only (advection disabled by setting $\velFactor = 0$ and $\mass = 0$). The object's skeleton acts as a Dirichlet condition: for every skeleton point $\q$, the concentration is fixed to a constant value $\concentration_\text{object}(\q) = C_s$ with $C_s > 0$. 
...
\begin{align}
    \eqlabel{eq:env-obj-reaction-diffusion-for-distances}{Screened diffusion for distance approximation}
...
\begin{algorithm}%[H]
    ...
    \KwIn{Screened field $\concentration_{\text{object}}(\cdot)$ with Dirichlet $\concentration_{\text{object}}=C_s$ on skeleton $\Gamma$, query point $\p_0$}
    \KwOut{Approximated distance $\hat d$}
    \BlankLine
    $\p \gets \p_0$\;
    \While{$\concentration_{\text{object}}(\p) < C_s - \varepsilon$}{
        $\p \gets \p + \eta \nabla \concentration_{\text{object}}(\p)$ \tcp*{gradient ascent}
    }
    \Return{$\hat d \gets \|\p - \p_0\|$}
\end{algorithm}
...
\subsubsection{Height modifiers}
...
\AltTextImage{
    ...
    \begin{align}
        \eqlabel{eq:env-obj-coarse-heights-computations}{Coarse height aggregation by group}
    ...
    \begin{align}
        \eqlabel{eq:env-obj-coarse-height}{Final coarse elevation}
        \height^{\star}(\p)=
...
}{height_modif.png}{The final \gloss{CoarseHeight} is a combination of the three groups of \glosses{EnvObj}: grounded data $\groundedHeight$ (red) and altitude-relative shapes $\altitudeHeight$ (orange) are aggregated by the $\smoothmax$ operator, then the surface elements $\surfaceHeight$ (green) are added.}{fig:env-obj-height-modif}{Combination of grounded, altitude-relative, and surface \glosses{EnvObj} in coarse elevation computation}

\subsubsection{Influence on water currents}
\label{sec:env-obj-water-currents}

We define the global water current as a vector field composed of three components:
\begin{align}
    \eqlabel{eq:env-obj-water-flow-expression}{Water velocity field decomposition}
    \Water(\p) = \Wuser(\p) + \Wsimu(\p) + \Wobj(\p).
\end{align}

Here, $\Wuser(\p)$ is a user-defined vector field that provides direct control over the flow direction and intensity using stroke paths, as presented in the previous chapter. The term $\Wsimu(\p)$ is an analytically defined component directly adopted from the terrain-aware wind simulation approach introduced by\citeName{Paris2019b} \cite{Paris2019b}, which we use for large water body fluid simulation. Finally, $\Wobj(\p)$ represents the deformation introduced by environment objects.

The simulated component $\Wsimu$ captures how water flow is diverted and shaped by the underlying terrain. It starts from a uniform flow direction $a$ and introduces warping based on the terrain gradient evaluated at multiple spatial scales (\cref{eq:env-obj-water-simulation-warping}). The resulting vector field is expressed as:
\begin{align}
    \eqlabel{eq:env-obj-water-simulation-main}{Terrain-aware analytic flow}
...
\begin{align}
    \eqlabel{eq:env-obj-water-simulation-venturi}{Venturi-style velocity scaling for \cref{eq:env-obj-water-simulation-main}}
...
\begin{align}
    \eqlabel{eq:env-obj-water-simulation-warping}{Gradient-based flow warping}
...
\AltTextImage{
    \subsubsubsection{Vector field deformation through Kelvinlets}

    $\Wobj$ is a deformation field defined as the accumulation of flow primitives. The use of analytical primitives to represent localised variations within large-scale scenes has been explored in various contexts through the edition of wind fields (\cref{fig:env-obj-wejchert-flow}) \cite{Wejchert1991} and authoring of rivers' water surface geometry (\cref{fig:env-obj-peytavie-river-primitives}) \cite{Peytavie2019}. In the latter, flow primitives are primarily used to generate the geometry of the water surface, but the same primitives are also reused to produce a flow field, enabling effects such as floating debris or leaves drifting along the surface. Because geometry is sensitive to overlapping influences, the authors organise primitives into a blending tree, where merge and replace operations control how individual contributions interact.
}{riverscape-primitives-Peytavie2019.png, riverscape-result-Peytavie2019.png}{\citeName{Peytavie2019} uses a sparse representation of a river surface to include details at the water surface that are aggregated in a construction tree \cite{Peytavie2019}.}{fig:env-obj-peytavie-river-primitives}{\cite{Peytavie2019}'s sparse representation of river surface}

\AltTextImage{
    On the other hand, \citeName{Wejchert1991} showed that simply summing flow primitives generates velocity fields that approximate Navier-Stokes dynamics (\cref{fig:env-obj-wejchert-flow}) \cite{Wejchert1991}. Because this method affects motion rather than visible geometry, this additive approach offers a good trade-off between plausibility, user control, and computation time.
...
}{aerodynamics-primitives-Wejchert1991.png, aerodynamics-addition-Wejchert1991.png}{\citeName{Wejchert1991} propose to describe the flow field of the environment by a sum of primitives, resulting in a simulation of wind field controllable by the user at very small memory cost \cite{Wejchert1991}.}{fig:env-obj-wejchert-flow}{\cite{Wejchert1991}'s flow primitives}
...
To regularise the singularity at $\p = \q$, \citeName{DeGoes2017} introduced a smoothed form known as the regularised Kelvinlets, which allow deformation effects to fall off smoothly with distance, and prevent numerical instabilities \cite{DeGoes2017}.
...
\AltTextImage{
    We use the scale, grab, and twist formulations of the regularised Kelvinlets brushes (\cref{fig:env-obj-kelvinlets-demo}), denoted as $s_\eps(\tensor{r})$, $g_\eps(\tensor{r})$, and $t_\eps(\tensor{r})$ respectively, to simulate obstruction, diversion, and vortex formation. Each \gloss{EnvObj} may combine several instances of these primitives at different positions and orientations.:
    \begin{align}
        \eqlabel{eq:env-obj-kelvinlet-scale-grab-expressions}{Regularised Kelvinlets scale and grab operators}
    ...
    \frac{a \eps^2}{2 r_\eps^3} \identity \right] \force, \nonumber \\
        t_\eps(\tensor{r}) &= -a\left(\frac{1}{r_\eps^3} + \frac{3 \eps^2}{2 r_\eps^5} \right) \q \times \tensor{r},
    \end{align}
...
}{kelvinlets-grab-demo-DeGoes2017.png, kelvinlets-demo-DeGoes2017.png}{\citeName{DeGoes2017} presents four types of deformations using Kelvinlets, resulting in organic pinch-like interaction with the original field. Top: a grab on a \textcopyright Disney/Pixar character. Bottom, from left to right: a twist, a scale, and a pinch operation \cite{DeGoes2017}.}{fig:env-obj-kelvinlets-demo}{\cite{DeGoes2017}'s Kelvinlet deformations}

Deformations defined on curves use $\q = \closestCp$ with $\closestCp$ the closest point on the curve from the point $\p$ and $\force = \curve'(\p)$. We then obtain each \gloss{EnvObj}'s flow field as the sum of all its Kelvinlet primitives distributed according to the \gloss{EnvObj}s shape and influence: $\tensor{u}_\oneObject(\p) = \sum{s_\eps} + \sum{g_\eps} + \sum{t_\eps}$.
...
\begin{align}
    \eqlabel{eq:env-obj-water-object-expression}{Object-induced flow contribution}
    \Wobj(\p) = \sum_{\oneObject \in \objects}^{}{\lambda_\oneObject \tensor{u}_\oneObject(\p)}.
\end{align}

The different configurations of Kelvinlet compositions used for point-, curve-, and region-based objects are illustrated in \cref{fig:env-obj-objects-and-kelvinlets}, showing how each \gloss{EnvObj} modifies the base flow according to its geometry and role in the environment.
...
\cref{fig:env-obj-rock-with-kelvinlets} presents a rock acting as an obstacle to the incoming water flow, which guides the flow, and more importantly the \glosses{EnvMat}, in a plausible direction on the side of the obstacle, with the introduction of artificial vortices at its back. At a larger scale, the same additive formulation extends to multiple interacting environment objects. \cref{fig:env-obj-island-flowfield} shows how islands, reefs, and coastal structures jointly reshape the flow, their individual Kelvinlet-based influences blending into a coherent global current pattern. A curve-based water influence is presented in \cref{fig:env-obj-canyon-flow} and \cref{fig:env-obj-canyon-inside-with-arch}, which is a unique "grab Kelvinlet" in the direction of the curve skeleton of the canyon, and thus inserting new \glosses{EnvObj} downstream, where the \glosses{EnvMat} are transported.

To assess the plausibility of our analytical formulation, \cref{fig:env-obj-barrier-flowfield} compares our 2D projection of the velocity field around a reef with a full 3D CFD simulation, showing that the principal flow deflections and side vortices are well reproduced.

Finally, \cref{fig:env-obj-archipelago-flowfield} demonstrates the scalability of the approach to larger scenes such as an archipelago. Each island and reef acts as an independent Kelvinlet composition, collectively producing a plausible large-scale oceanic flow without requiring full-domain simulation.


\begin{figure}
    \autofitgraphics[]{objects-kelvinlets.pdf}
    \caption[Examples of \glosses{EnvObj}' Kelvinlet composition]{Examples of point-, curve- and region-based \glosses{EnvObj}' Kelvinlet operators composition. The $\Wobj$ field modifies the initial water flow. Final water flow is reprented by streamlines (dashed grey arrows).}
    \label{fig:env-obj-objects-and-kelvinlets}
\end{figure}

\begin{figure}[b]
    \autofitgraphics[]{2025-06-21__12-53-48_render_top_view-with-sand.png, 2025-06-21__12-53-48_render_top_view-with-flow-with-sand-with-annotations.png, simu-cylinder.png}
    \autofitcaptions[]{Materials steady state, Kelvinlets used, Ground truth}
    % \autofitgraphics[]{2025-06-21__12-53-48_render_top_view-with-sand.png, 2025-06-21__12-53-48_render_top_view-with-flow-with-sand.png, 2025-06-21__12-53-48_render_top_view-with-flow-with-sand-with-annotations.png, simu-cylinder.png}
    % \autofitcaptions[]{Materials steady state, Rock velocity field, Kelvinlets used, Ground truth}
    \caption[Visualisation of water currents deviated by a rock]{A strong current is affected by a rock, visible by the deviation of sand (green) produced by smaller rocks. The flow is directly computed by the composition of three Kelvinlet objects assigned to the rock: a "scale Kelvinlet" (yellow) deflects the materials as an obstacle would; and two "twist Kelvinlets" (cyan) approximate the vortices of the flow, pushing the sand back behind the rock. Rightmost displays streamlines of CFD on a cylinder, matching approximatively our analytical solution. }
...
\begin{figure}
    ...
    \caption[Combination of currents modifications with multiple \glosses{EnvObj}]{Each \gloss{EnvObj} has an influence on the water currents. Islands, defined with an area, add a force towards themselves (waves crashing on coasts) and simultaneously a force towards the ocean that acts as an obstacle flow. The resulting vector field of the scene is the sum of each \gloss{EnvObj}'s influence.}
    \label{fig:env-obj-island-flowfield}
\end{figure}

\begin{figure}[b]
    \autofitgraphics[]{simu-barrier.png, simu-barrier-results.png}
    \autofitcaptions[]{CFD ground truth, Our result}
    % \autofitgraphics[]{simu-barrier-3D.png, simu-barrier.png, simu-barrier-results.png}
    % \autofitcaptions[]{3D ground truth, 2D ground truth, Our result}
    \label{fig:env-obj-barrier-flowfield}
    \caption[Water flow for a reef]{3D fluid simulation over a reef. Our method approximates the 2D projection of this output, with the introduction of turbulences on the side of the reefs.}
\end{figure}

\begin{figure}
    \autofitgraphics[]{islands-waters-no-flow.png, islands-waters-with-flow.png}
    \autofitcaptions[]{\Glosses{EnvObj}, Resulting water flow field}
    \caption[Water flow for an archipelago]{Archipelago with few islands, reefs and individual corals under an initial uniform water flow (going bottom to top) result in an analytical flow with each \gloss{EnvObj} composed of Kelvinlet operators. }
    \label{fig:env-obj-archipelago-flowfield}
\end{figure}
...
\subsubsection{Direct interactions with the \glosses{EnvObj}}
...
The deformation of \glosses{EnvObj} is applied on curve and region \glosses{EnvObj} by displacing directly the control points of the skeleton (\cref{fig:env-obj-user-interaction} deforming a canyon, and \cref{fig:env-obj-island-deformation} editing an island). 
%Various methods are suitable for displacing a polygon, such as the use of Kelvinlets, cage deformation, displacement maps, or Gaussian influence. In the scope of this chapter, we will not cover the multiple aspects of curve and polygon deformation. 
As the closed or open geometry shape of the \gloss{Skeleton} changes, we run again the \gloss{FittingFunc} optimisation through our Snake algorithm (\cref{sec:env-obj-generation-rules}).

Locking the vertices from the deformed \gloss{Skeleton} edited manually by the user is achieved by cancelling the energy $\energy(\curve)_v$ from the Snake algorithm for this specific vertex $v$, disabling it from any displacement during the optimisation process. Finer control can be given by applying a coefficient on the energy gradient, such that the vertices are not locked but their displacement is only hindered.

By modifying an \gloss{EnvObj}, \glosses{EnvVal} may change, which can result in the destruction of the now incompatible \glosses{EnvObj} in the scene (\cref{fig:env-obj-user-interaction}, middle). The removal of \gloss{EnvObj} is subject to chain reactions, often visible in nature. Reducing the reaction is achievable by alternating, until a steady state is reached again, a step in the environment update (\cref{sec:env-obj-materials}) and a step of \gloss{Skeleton} refinement for each \gloss{EnvObj}. As a counterbalance to the removal, each deleted \gloss{EnvObj} is reinserted if possible in the scene (\cref{fig:env-obj-user-interaction}, right) following the process described in \cref{sec:env-obj-generation-rules}.

\begin{figure}[b]
    \autofitgraphics[]{InteractionEdition1.png, InteractionEdition2.png, InteractionEdition3.png}
    \caption[Corals next to a canyon responding to the user displacing it]{Starting from a coral colony developed around a canyon (left), the user edits the shape of the canyon, resulting in a different configuration of the scene, killing the corals that end up too deep in the water (centre) and the development and growth of new corals at the previous location of the canyon (right).}
...
\begin{figure}
    ...
    \caption[Deformation of an \gloss{EnvObj} is applied by displacing the control points of its outlines.]{Deformation of an \gloss{EnvObj} is applied by displacing the control points of its outlines. All \glosses{EnvObj} in the scene evaluate their new fitness in the environment.}
...
\begin{figure}[b]
    ...
    \caption[A strong water current created by the user has a direct influence on the corals that are sensitive.]{A strong water current created by the user has a direct influence on the corals that are sensitive. \Glosses{EnvObj} that are far enough from the user stroke are unaffected.}
    \label{fig:env-obj-user-flow-effects}
\end{figure}

\subsubsection{Indirect interaction with \glosses{EnvObj}}
\label{sec:env-obj-events}
A configuration file defines in advance the different \glosses{GeoEvent} that should be triggered during the simulation. 
...
Multiple \glosses{GeoEvent} can be triggered either as sudden or continuous environmental changes. These changes have a major impact on landscape morphology.
We define \glosses{GeoEvent} with a starting point and an ending point, such that at any time of the simulation we compute the progress of the \gloss{GeoEvent} as $\tEvent \in [0, 1]$ and linearly interpolate the effects on \glosses{EnvVal}.

\subsubsubsection{Water level events}
Water level changes are key \glosses{GeoEvent} that shape underwater landscapes. As previously submerged \glosses{EnvObj} become elevated above the water level, flora and fauna terrain features dry and die. Deprived of the living part of the features, everything is more affected by terrestrial erosion. By updating the value of the depth $\depth$ evaluated in the \glosses{FitnessFunc}, any \gloss{EnvObj} that is sensitive to depth and altitude are automatically impacted, which may cause death (\cref{fig:env-obj-water-event}). The modification of the water level is defined as:
\begin{align}
    \eqlabel{eq:env-obj-event-depth-changes}{Depth update under water-level events}
...
\begin{figure}
    ...
    \caption[User interaction with a scene by modifying the water level]{Lowering the water level by a few metres caused most of the coral objects to satisfy $\fitnessFunc \leq 0$, causing their death. Since the water level (blue) decreases slowly, new coral objects spawn progressively at a lower altitude.}
    \label{fig:env-obj-water-event}
\end{figure}

\subsubsubsection{Subsidence and uplift events}
Subsidence and uplift are the main \glosses{GeoEvent} that create or destroy islands in the long term, as presented in \cref{chap:coral-island}. These \glosses{GeoEvent} are simulated as a simple factor on the height field of the generated terrain (\cref{fig:env-obj-subsidence-event}). Subsidence is not always uniform across the terrain. As such, the user can provide a position $\q$ at which the subsidence is strongest, the amount of subsidence applied $\Delta \height_e$ and a standard deviation $\std$, from which we then compute, at any point in space and time of the simulation, the height of the terrain:
\begin{align}
    \eqlabel{eq:env-obj-event-heightchanges}{Height update under subsidence/uplift}
...
\end{align*}
The Gaussian analytic formulation guarantees continuity of the changes on the \glosses{EnvVal}, with smooth transitions from the epicentre of the \glosses{GeoEvent} as the distance increases. Any other analytical formulation could be used, but we chose the Gaussian formulation for its simplicity since the only parameter needing parametrisation is the standard deviation $\std$.

\begin{figure}
    \autofitgraphics[]{InteractionSubsidence1.png, InteractionSubsidence2.png}
    \caption[User interaction with a scene by causing subsidence]{Simulating subsidence on a part of the terrain (brown area) causes depth value to change locally, resulting in the death of coral objects that find themselves too deep to survive. Here two subsidence \glosses{GeoEvent} are triggered in parallel.}
    \label{fig:env-obj-subsidence-event}
\end{figure}

\subsubsubsection{Storm events}
Storms are significant factors in the geomorphology of coral reefs \cite{VilaConcejo2016, Oron2023} and coasts \cite{Dominguez2005, Cowart2010}. Due to the extreme wind and wave velocities, coasts are highly eroded in a short time period and the more fragile corals near the water surface are broken, possibly causing breaches in the reefs and spreading polyps in the current's direction. While there are various factors at play to understand the appearance of storms and the hydrodynamics affecting them, we simplify the model of storms to a single epicentre $\q$ with a wind velocity $\windVelocity$ and a standard deviation $\std$ representing the spread around the epicentre (\cref{fig:env-obj-storm-event}). The computation of water currents is then given as:
\begin{align}
    \eqlabel{eq:env-obj-events-water-changes}{Flow field with storm events}
...
\begin{figure}[b]
    ...
    \caption[User interaction with a scene by creating a storm]{The result of a storm localised on one side of the island (red area) modifies the result of the evaluation of \glosses{EnvObj} around its epicentre for a short period of time. Most of the coral objects died from the \gloss{GeoEvent}, except for a few \glosses{EnvObj} less sensitive to the strength of the water currents.}
    \label{fig:env-obj-storm-event}
\end{figure}

\midConclusion

Our framework can easily be extended as \gloss{GeoEvent} system remains similar for all \glosses{GeoEvent} as we see in \cref{alg:env-obj-geoevents}. Including higher-level simulations in the \gloss{GeoEvent} system can be added, such as the simulation of tectonic activity, the use of fluid dynamics for tsunami \glosses{GeoEvent}, the integration of human activity to alter specific \glosses{EnvMat}, etc...

\begin{algorithm}%[H]
    \DontPrintSemicolon
    \caption{Applying \glosses{GeoEvent}.}
    \label{alg:env-obj-geoevents}
    \KwIn{$\height,\Wlevel,\Wuser$, events, $t$}
    \KwOut{$\height,\Wlevel,\Wuser$ after applying \glosses{GeoEvent} changes}
    \BlankLine
    \ForEach{$e$ in events}{$\tEvent \gets \frac{t - t_{e, \text{start}}}{t_{e, \text{end}} - t_{e, \text{start}}}$}
...
\subsection{Large-scale generation}

\begin{figure}[b]
    \autofitgraphics[]{2025-06-20__15-54-59_render_top_view-with-flow-with-sand.png, 2025-06-20__15-57-26_render_top_view-with-sand.png, 2025-06-20__15-58-34_render_top_view-with-sand.png, 2025-06-20__16-01-26_render_top_view-with-deadcoral.png, 2025-06-20__16-04-20_render_top_view-with-deadcoral.png}
    \autofitgraphics[]{2025-06-20__15-54-59_input_label.png, 2025-06-20__15-54-59_input_label.png, 2025-06-20__15-57-26_input_label.png, 2025-06-20__15-58-34_input_label.png, 2025-06-20__16-04-20_input_label.png}
    \caption[Iterative construction of an island under high wind force]{Iterative construction of an island under strong prevailing wind.
    Sand is advected along the wind direction, forming an elongated beach ridge that shelters a low-energy lagoon.
    Coral colonies subsequently develop along the lagoon rim where sedimentation is minimal.
    Bottom: label map extracted from the \glosses{EnvObj} of the scene.}
    \label{fig:env-obj-iterative-island}
\end{figure}

In the example presented in \cref{fig:env-obj-iterative-island}, we start with a single "Island" \gloss{EnvObj} that diffuse sand, and a uniform wind that sets a transport direction. Sand moves according to the advection-diffusion-reaction model: advection couples sand to water flow field, diffusion smooths local variations, and decay drives a steady state.
...
Once the lagoon is present, we instantiate "Coral colonies". Their \gloss{FitnessFunc} combines a preference for moderate currents with an avoidance of sand concentration and large quantities of "Coral polyp"; the \gloss{FittingFunc} keeps them between water level $\Wlevel$ and 10m deep $\Wlevel - 10$ . Colonies deposit "Coral polyp" and "Dead coral", a calcareous material that allows the instantiation of a "Coral reef". The reef then acts as an obstacle, further calming the lagoon and closing the loop between morphology and ecology.

\subsection{Medium-scale generation}

In \cref{fig:env-obj-canyon-flow} and \cref{fig:env-obj-canyon-inside-with-arch} the substrate contains a noisy "hard bedrock" material with no diffusion nor decay (constantly present). The canyon is a curve-based \gloss{EnvObj} whose \gloss{FittingFunc} follows a path that minimises hard rock so it travels through softer corridors. Its water influence creates a flow lane along its path (\cref{fig:env-obj-canyon-flow}).


"Arches" are point-based \glosses{EnvObj} with \gloss{FitnessFunc} maximised with current strength and bedrock abundance, making their seed placement in the canyon. The \gloss{FittingFunc} favours deeper spots, moving the seed placement in a local depth maximum, inside the canyon. An arch produces shade and deposits a "rock fragment" material. That deposit "Pebble" material, spawning "Rock" objects, which in turn act as sources for sand. \cref{fig:env-obj-canyon-scene} adds smaller rocks at each iteration.

The result is a spawning cascade, with canyons allowing the instantiation of arches, which in turn generate rocks.
% The result is layered and readable: arches at hard–soft interfaces under strong flow; rocks and pebbles beneath; sand ripples in low-energy pockets (\cref{fig:env-obj-canyon-inside-with-arch}).

\begin{figure}[b]
\autofitgraphics[]{2025-06-20__10-32-20_render_top_view.png, 2025-06-20__10-32-20_render_top_view-with-flow.png}
\caption[A canyon creates a water current in its direction]{Water flow oriented along the canyon curve. The analytic flow field aligns currents with the canyon's skeleton direction.}
...
\begin{figure}[t]
    ...
    \caption[Inside view of a canyon with an arch.]{Inside view of a canyon with an arch.
    The arch produces shade (red) and pebbles (green), creating favourable conditions for rocks and sand deposition.}
    \label{fig:env-obj-canyon-inside-with-arch}
\end{figure}

\begin{figure}[t]
    \autofitgraphics[]{2025-06-20__10-36-21_render_top_view-with-sand.png, 2025-06-20__10-38-05_render_top_view-with-sand-small-decay.png, 2025-06-20__10-39-13_render_top_view-with-sand-no-water-transport.png, 2025-06-20__10-40-44_render_top_view-with-sand-high-diffusion.png, 2025-06-20__10-42-22_render_top_view-with-sand-reverse-water-transport.png}
    \caption[Influence of material parameters on their spread]{Influence of material parameters on sand distribution.
    From left to right, and top to bottom: reference configuration, low decay, no water transport, high diffusivity, and reversed water flow.}
    \label{fig:env-obj-material-diffusion-parameters}
\end{figure}

We also vary the transport model to probe sensitivity (\cref{fig:env-obj-material-diffusion-parameters}). Lower decay spreads sand farther; high diffusivity blurs contrasts; removing or reversing the water-driven advection flips directionality but remains stable. The steady-state formulation gives predictable, controllable patterns.



\begin{figure}[t]
    \autofitgraphics[]{Canyon2.png, Canyon3.png, Canyon4.png, Canyon5.png}
    \caption[Evolution of a canyon scene at different iterations of the simulation]{Evolution of a canyon scene at different iterations.
    The formation of an arch triggers rock and pebble spawning, followed by sand deposition and ripple emergence at the canyon base.}
    \label{fig:env-obj-canyon-scene}
\end{figure}

\subsection{Small-scale generation}

We present two complementary setups that use region-based \glosses{EnvObj} and simple material feedback to produce self-organised patterns.


In \cref{fig:env-obj-coral-colonization-scene}, three generic coral colonies "Coral A", "Coral B", and "Coral C" are restricted to a user-drawn control field $\influence$ in the shape of an annulus. Each species deposits generic \gloss{EnvMat} "Polyp" and its own \gloss{EnvMat} "Polyp A", "Polyp B", or "Polyp C". The \gloss{FitnessFunc} $\fitnessFunc_A = \frac{\text{Polyp}}{\text{Polyp A}}$ depends only on these materials: self-material raises fitness; other species materials reduce it. Starting from random seeds, colonies expand until their fronts meet. At the border between two colonies, none of the colonies make progression due to the amount of coral polyp specific from the other colony.

\begin{figure}
\autofitgraphics[]{focusArea.png, col0.png, col1.png, col2.png, col3.png}
\caption[Three colonies of coral restricted to an annulus fighting for space]{Three colonies of coral (red, blue, green) restricted to an annulus (leftmost).
The colonies expand until their borders meet, forming stable boundaries where no further progression occurs.}
\label{fig:env-obj-coral-colonization-scene}
\end{figure}

In \cref{fig:env-obj-coral-layout} we assign ecological preferences to different coral types via their \gloss{FitnessFunc}: preferred depth band, a current range that balances supply and stress, a slope tolerance, and an aversion to sand. Each coral entity finds a place that fits the distribution of species presented in \cref{sec:state-of-the-art_biology}. 

\begin{figure}[b]
    \autofitgraphics{islands_with_different_coral_types.png}
    \caption[Organic layout of coral species from generation rules]{Distribution of coral morphotypes from generation rules.
    Branching corals (purple) dominate back reefs, foliose corals (cyan) the reef crest, and massive corals (brown) extend toward the fore reef.}
...
The proposed method aims to generate plausible landscapes using simplified versions of the evolution of an ecosystem and of the 3D representation. The biological realism of the result is highly correlated to the amount of simplification and assumptions, while the visual realism is completely dependent on the geometric functions used for the 3D modelling of the \glosses{EnvObj}. We propose a flexible generic approach for terrain generation where close collaboration with field experts and with graphists is needed to achieve optimal results.
...
% \begin{figure}
%     \autofitgraphics[]{2025-06-20__10-32-20_render_top_view.png, 2025-06-20__10-32-20_render_top_view-with-flow.png }
%     \caption[A canyon creates a water current in its direction.]{A canyon creates a water current in its direction.}
%     \label{fig:env-obj-canyon-flow}
% \end{figure}

% \begin{figure}
%     \autofitgraphics[]{2025-06-20__10-32-42_render_inside_view.png, 2025-06-20__10-33-11_render_inside_view-with-boulders.png, 2025-06-20__10-33-11_render_inside_view-with-shade.png, 2025-06-20__10-36-21_render_inside_view-with-sand.png}
%     \caption[Inside view of a canyon with an arch.]{Inside view of a canyon with an arch. The arch generates shade (red) and pebbles (green), which is a beneficial environment for the appearance of rocks, which will in turn spread sand.}
%     \label{fig:env-obj-canyon-inside-with-arch}
% \end{figure}
...
%     \caption[Influence of material parameters on their spread]{Influence of material parameters on their spread (sand generated by the rocks). From left to right, and top to bottom: normal sand parameters, low decay, no influence of water flow, large diffusivity, inverse water influence (spread upwind in the canyon).}
%     \label{fig:env-obj-material-diffusion-parameters}
% \end{figure}
...
% Following these simple rules, \cref{fig:env-obj-canyon-scene} shows the emergence of localised details in the scene.

% \begin{figure}
%     \autofitgraphics[]{Canyon2.png, Canyon3.png, Canyon4.png, Canyon5.png}
%     \caption[Evolution of a canyon scene at different iterations of the simulation]{Evolution of a canyon scene at different iterations of the simulation. The appearance of an arch causes the spawning of rocks, pebbles, and finally some deposition of sand at the bottom of the canyon, spawning ripples.}
%     \label{fig:env-obj-canyon-scene}
% \end{figure}
...
% \begin{figure}
%     \autofitgraphics[]{focusArea.png, col0.png, col1.png, col2.png, col3.png}
%     \caption[Three colonies of coral restricted to an annulus fighting for space]{Three colonies of coral (red, blue, green) restricted to an annulus (leftmost, red has negative impact on the fitness function while green has positive impact) in the middle section of the terrain fighting for the space.}
%     \label{fig:env-obj-coral-colonization-scene}
% \end{figure}

% \begin{figure}
%     \autofitgraphics{islands_with_different_coral_types.png}
%     \caption[Organic layout of coral species from generation rules]{Introducing penalities on water currents and terrain slope for different types of corals, we see naturally a layout emerging with branching corals (purple) in the back reef, foliose corals (cyan) mainly around the reef crest, and massive corals (brown) spreading from the back reef to the fore reef. }
%     \label{fig:env-obj-coral-layout}
% \end{figure}
...
This chapter introduces a procedural terrain generation method grounded in a sparse and semantic representation of landscapes.
This framework, centred on the \glosses{EnvObj}, provides a unified abstraction for terrain and ecosystem features through parametric points, curves, and regions.
By embedding expert knowledge into the \glosses{FitnessFunc} and maintaining a steady-state formulation of the environment, our approach allows both automatic simulation and interactive user guidance, while preserving coherence across multiple scales.
Each \gloss{EnvObj} locally modifies and queries environmental fields, enabling a computationally efficient yet lifelike synthesis of terrains and ecosystems.

Despite these strengths, several aspects of the current approach remain simplified and open research directions:

\paragraph{Dimensional simplification. }
Our framework relies on two-dimensional scalar and vector fields, which ensures efficiency and interpretability but restricts the representation to heightfield-like surfaces.
Extending this model to volumetric environmental fields would enable the generation of more complex geological and biological structures such as caves, overhangs, and the internal architecture of coral reefs.

\paragraph{Dynamic evolution.}
The current formulation focuses on steady-state snapshots of plausible environments rather than on their continuous evolution.
Incorporating dynamic processes including erosion, sediment transport, or biological growth, would bridge the gap between procedural generation and environmental simulation, allowing the depiction of terrains that change over time under natural forces.

% Third, while the analytic flow formulation and simplified transport equations provide plausible hydrodynamic patterns, they remain approximations of physical processes.
% Coupling these analytic methods with localised physics solvers or data-driven corrections could enhance realism in critical zones, such as coastal regions or areas affected by strong currents, without compromising interactivity.

\paragraph{Shape optimisation and structural realism.}
In the current implementation, the optimisation of \glosses{EnvObj} geometry is guided by target parameters such as a desired length or surface area, which ensures controllability but does not reflect biological or geomorphological growth patterns.
Future work could investigate alternative approaches to structure generation, for instance by extracting medial-axis or skeleton structures from fitness fields to instantiate new objects, rather than relying solely on our Snake optimisation, or to be used as the initial shape for our Snake optimisation. 
Such biologically inspired mechanisms could produce more natural shapes and spatial organisations.

\paragraph{Direct interaction and deformation.}
The deformation of curve and region \glosses{EnvObj} is currently achieved by directly displacing the control points of their skeletons, after which the \gloss{FittingFunc} optimisation is rerun.
More advanced deformation models such as sharp Kelvinlets, cage-based deformation, or Gaussian influence fields, could improve the continuity of user edits, allowing intuitive reshaping while preserving physical and ecological constraints.

\paragraph{Expert knowledge requirements.}
The plausibility of the results currently depends on expert-defined parameters that require manual tuning.
Future work could explore learning-based estimation or inverse modelling strategies to derive these parameters automatically from real data or target exemplars, thus improving generality and ease of use.

% Finally, while our sparse, semantic representation facilitates intuitive editing, further developments should focus on integration and interoperability with existing authoring tools and simulation frameworks.
% Representing \glosses{EnvObj} and \glosses{EnvVal} as standardised layers would enable collaboration between artists, scientists, and engineers, reinforcing the bridge between creative design and environmental modelling.

\smallConclusion

In summary, the \glosses{EnvObj} framework offers a semantic, sparse, and interactive foundation for procedural terrain and ecosystem generation.
By combining rule-based reasoning with analytical field interactions, it achieves a balance between plausibility, control, and performance.
Its current simplifications opens the way toward richer extensions that integrate volumetric representations, evolutionary processes, and data-driven calibration. 

Because the framework does not explicitly model surface geometry, the resulting terrains can appear synthetic and lack fine-scale realism.
The following chapter addresses this limitation by introducing an erosion simulation method based on particle simulation.
This approach generalises detachment, transport, and deposition processes across both surfacic and volumetric domains, providing a flexible and efficient mechanism for integrating erosion into procedural terrain generation.
...
%     \caption[A simple coral reef island is generated using an island, a lagoon, reefs, coral polyps, beaches, trees and algae \glosses{EnvObj}]{A simple coral reef island is generated using an island, a lagoon, reefs, coral polyps, beaches, trees and algae \glosses{EnvObj}. Trees appear on beaches and algae grow in the lagoon's sand.}
