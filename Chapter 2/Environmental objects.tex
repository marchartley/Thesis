...
The use of \glosses{EnvMat} provides an efficient, on-demand approximation of the distance to the closest instance of an object type $\objects$, with computational complexity linear in the number of objects in the scene. For each object type (e.g., coral, rock, reef, island, ...), we maintain a separate \gloss{EnvMat} field $\material_\oneObject$ that evolves under diffusion-reaction only (advection disabled by setting $\velFactor = 0$ and $\mass = 0$). The object's skeleton acts as a Dirichlet boundary: for every skeleton point $\q$, the concentration is fixed to a constant value $\concentration_\oneObject(\q) = C_s$ with $C_s > 0$. 
...
\begin{align}
    \concentration(\mathbf x,t+\Delta t)
    &=\concentration(\mathbf x,t)
      + \Delta t \left(\diffusion\nabla^2 \concentration(\mathbf x,t)
      - \decay\,\concentration(\mathbf x,t)\right) \\
      %\quad \mathbf x \notin \Gamma, \\
    \concentration(\q,t) &= C_s, \quad \q \in \Gamma.
\end{align}

To approximate the distance from a query point $\p_0$ to the nearest object, we perform gradient ascent on the concentration field: starting at $\p_0$, we follow the discrete gradient $\nabla \concentration$ until reaching a location $\p$ where $\concentration(\p)=C_s$. The distance estimate is then
\begin{align}
    d \approx \|\p-\p_0\|.
\end{align}
