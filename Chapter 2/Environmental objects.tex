...
\begin{algorithm}%[H]
    ...
    \While{\text{not stop (max iters / $\targetObjects$ reached / user approval)}}{
        % Instantiation
        \tcp{(1) Instantiate new objects (\cref{sec:env-obj-generation-rules})}
        \ForEach{$\text{class} \in \availableObjects$}{
            sample position: $\p_s \gets \arg\max_{\p_k}\;\fitnessFunc_{\text{class}}(\p_k,\environment)$ \tcp*{\cref{sec:env-obj-fitness-func}}
            \If{$\fitnessFunc_{\text{class}}(\p_s,\environment) > threshold_{\text{class}}$}{
            compute object's \gloss{Skeleton}: $\gloss{Skeleton}_o \gets \fittingFunc_{\text{class}}(\environment)$ \tcp*{\cref{sec:env-obj-fitting-func}}
            add new object $o$ to $\objects$\;
            }
        }

        % Environment update
        \tcp{(2) Environment update from \glosses{EnvModif} (\cref{sec:env-obj-materials})}
        $\height \gets \text{recompute from all objects height modifiers } \heightModif_o$ \tcp*{\cref{sec:env-obj-height-modifiers}}
        $\Wsimu \gets \text{recompute from } \height$ \tcp*{\cref{sec:env-obj-water-currents}}
        $\Wobj \gets \sum_{o \in \objects}\lambda_o\,\tensor{u}_o$\;
        $\Water \gets \Wuser + \Wsimu + \Wobj$\;
        $\material \gets \text{advection-reaction-diffusion}$ \tcp*{\cref{sec:env-obj-material-modifiers}}
        $\environment \gets (\height,\;\Water,\;\Wlevel,\;\material)$\;
        apply direct user edits to \gloss{Skeleton} \tcp*{\cref{sec:env-obj-manual-interaction}}
        apply \glosses{GeoEvent} analytically on $\environment$ \tcp*{\cref{sec:env-obj-events}}
...
\paragraph{Approximation of distances from \glosses{EnvMat}.}
...
Deformations defined on curves use $\q = \closestCp$ with $\closestCp$ the closest point on the curve from the point $\p$ and $\force = \curve'(\p)$. We then define $\tensor{u}_\oneObject(\p) = s_\eps(\q - \p) + g_\eps(\p - \q)$.
