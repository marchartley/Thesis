...
\begin{Itemize}
    ...
    \[
        1 - e^{-kx} = kx + \frac{k^2 x^2}{2} + \cdots,
    \]
...
\begin{align}
    \frac{\partial^2}{\partial a^2} \smoothmax(a, b) = \frac{\partial^2}{\partial b^2} \smoothmax(a, b) = - f_2(a, b) \\
...
The expression is algebraically heavy, but all terms are composed of simple exponentials, products, and powers, making it fast and safe to compute in practice, and the redundant components an be cached. At $x = 0$, we have $h''(0) = 0$ by symmetry and smoothness of the underlying expression.
