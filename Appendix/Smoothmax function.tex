...
\teaser{
    ...
    \autofitcaptions[]{k=10, k=20, k=100, k=1000}
    \caption[Effect of $k$ on the \texttt{smoothmax} operator, illustrated in 3D]{Top: Smooth union of a sphere of radius 0.1 with a plane rendered with Shadertoy shows the effect of the sharpness parameter $k$. Lower values blends smoothly while larger values approximate the boolean union operator. Bottom: Multiple spheres blended together with the same setup with the same $k$ values by chaining the $\smoothmax$ operator. }
...
\begin{align}
    \eqlabel{eq:smoothmax-equation}{Definition of the Smoothmax operator}
...
\begin{align}
    \eqlabel{eq:smoothmax-f-function}{Auxiliary function for Smoothmax analysis}
    f(x) = \frac{x}{1 - e^{-kx}}.
\end{align}
...
Thus,
\begin{align}
    \eqlabel{eq:smoothmax-hopital-singularity}{L'Hôpital's rule at $x=0$ for the auxiliary function}
    \lim_{x \to 0} \frac{g'(x)}{h'(x)} = \lim_{x \to 0} \frac{1}{k e^{-kx}} = \frac{1}{k}.
\end{align}

Since the limit exists and equals $\frac{1}{k}$, we define
\begin{align}
    \eqlabel{eq:smoothmax-f-limit}{Limit of the auxiliary function at $x=0$}
    f(0) := \frac{1}{k}.
\end{align}

This definition removes the singularity and ensures that $f$ is continuous on $\R$. For the case $a = b$, we then have
\begin{align}
    \eqlabel{eq:smoothmax-a-equals-b}{Smoothmax limit for $a = b$}
    \smoothmax(a, b) = a + \frac{1}{2k}.
\end{align}

The complete definitions of the functions using \cref{eq:smoothmax-f-function,eq:smoothmax-f-limit} and \cref{eq:smoothmax-equation,eq:smoothmax-a-equals-b} are:
\begin{align*}
    % \eqlabel{eq:smoothmax-f-complete}{Final auxilary function $f$}
    f(x) &= \begin{dcases}
        \frac{x}{1 - e^{-kx}} &, x \neq 0 \\
        \frac{1}{k} &, x = 0
    \end{dcases}, \\
    % \eqlabel{eq:smoothmax-equation-complete}{Final Smoothmax function}
...
\begin{align}
    \eqlabel{eq:smoothmax-symmetry-check}{Symmetry condition for Smoothmax}
...
\begin{align}
    \eqlabel{eq:smoothmax-symmetry-auxilary}{Algebraic form used in the symmetry proof}
...
Developing \cref{eq:smoothmax-symmetry-check} and let $r = e^{2kx} > 0$:
\begin{align}
    \eqlabel{eq:smoothmax-symmetry-final}{Identity completing the symmetry proof}
    g(x) - g(-x) &= \frac{x}{1 - e^{-2kx}} - \frac{-x}{1 - e^{2kx}}  \\
    &= \frac{x}{(1 - e^{-2kx})} \cdot \frac{e^{2kx}}{e^{2kx}} + \frac{x}{1 - r} \\
...
\begin{align}
    \eqlabel{eq:smoothmax-equation-complete}{Piecewise definition of Smoothmax}
...
\begin{align}
    \eqlabel{eq:smoothmax-first-derivatives}{First derivatives of Smoothmax}
...
\begin{align}
    \eqlabel{eq:smoothmax-second-derivatives}{Second derivatives of Smoothmax}
...
\begin{itemize}
    ...
    \begin{align}
        \eqlabel{eq:smoothmax-LSE-formula}{Definition of LogSumExp (LSE)}
    ...
    \begin{align}
        \eqlabel{eq:smoothmax-AbsMax-formula}{Definition of Smooth Absolute Max (AbsMax)}
...
We compare these functions across four benchmarks: chaining behaviour, sharpness resolution, runtime performance, and aggregation error.

\subsection*{Chaining behaviour}
...
After deriving the formula in \cref{eq:smoothmax-symmetry-auxilary}:
\begin{align*}
