...
% First, let's exploit the radial symmetry of the metaball and rewrite $g(\p) = 1 - r$ by using the spherical coordinates of the point $\p - c$.
...
We use the following formula to evaluate a metaball in space with a center $c$ and radius $\Radius$:
\begin{equation}
    g(\p) = \max\!\left(1 - \frac{\abs{\p - c}}{\Radius},\,0\right),
\end{equation}
...
\begin{align}
    ...
    f(\p) &= \lambda\, g(\p), \\
    \label{eq:erosion-int_function_f_is_erosion}
    \int_{\p \in B_{\Radius}(c)} f(\p) \, d\p &= \totalErosion,
\end{align}
where $B_{\Radius}(c)$ denotes the ball of radius $\Radius$ centered at $c$.

Exploiting radial symmetry, we set $r = \abs{\p - c} / \Radius \in [0,1]$ and write $g(\p) = 1 - r$.  
In spherical coordinates of the normalized point $(\p - c)/\Radius$, the volume element is $\Radius^3\, r^2 \sin(\angl)\, dr\, d\angl\, d\anglTwo$.  
The integral of $g$ over $B_{\Radius}(c)$ becomes
\begin{align}
...
\begin{equation} 
    ...
    \max\!\left(1 - \frac{\abs{\p - c}}{\Radius},\,0\right),
\end{equation}
representing the rate of change applied to the terrain's evaluation function.

\subsection*{Derivatives}
Let $\rho = \abs{\p - c}$, $r=\rho/\Radius$, and $\vec{u} = (\p - c)/\rho$ (the unit radial vector).  
For $0 < r < 1$,
\begin{equation}
...
\begin{align}
    ...
    \qquad
    f(\p) = \lambda\,\phi\!\left(\frac{\abs{\p - c}}{\Radius}\right).
\end{align}

For implementation, it is convenient to record the radial derivatives once. Let $\rho=\abs{\p - c}$, $x=\rho/\Radius$, and $\vec{u}=(\p-c)/\rho$ (defined for $\rho>0$). For $0<x<1$,
\begin{equation}
    \nabla f(\p) = \frac{\lambda}{\Radius}\,\phi'(x)\,\vec{u},
    \qquad
    \Delta f(\p) = \frac{\lambda}{\Radius^2}\!\left[\phi''(x) + \frac{2}{x}\,\phi'(x)\right].
\end{equation}
By convention, we set $\nabla f(c)=0$. The smoothness at $x=0$ and $x=1$ depends on $\phi$; if $\phi'(0)\neq 0$ or $\phi'(1)\neq 0$, the gradient is discontinuous at the corresponding point.
...
\begin{minipage}[t]{0.5\linewidth}
    ...
    % \caption{#1 profile. Visuals are 2D slices through $c$; image parameters use $Q\equiv\totalErosion$ and $R\equiv\Radius$.}
  \end{figure}
  \end{minipage}
}
...
{ \begin{cases} 1-3q^2+2q^3, & 0\le x\le 1\\ 0,& x>1\end{cases} }
{ -6q+6q^2 }
{ -6+12q }
{ \tfrac{1}{15} }
...
{ \begin{cases} 1-10q^3+15q^4-6q^5, & 0\le x\le 1\\ 0,& x>1\end{cases} }
{ -30q^2+60q^3-30q^4 }
{ -60q+180q^2-120q^3 }
{ \tfrac{5}{84} }
...
{ \begin{cases} (1-x)^4(1+4q), & 0\le x\le 1\\ 0,& x>1\end{cases} }
{ -20q(1-x)^3 }
{ -20(1-x)^2(1-4q) }
{ \tfrac{1}{42} }
...
{ \begin{cases} (1-x^2)^2, & 0\le x\le 1\\ 0,& x>1\end{cases} }
{ -4q(1-x^2) }
{ -4+12q^2 }
{ \tfrac{8}{105} }
...
{ \begin{cases} (1-x^2)^3, & 0\le x\le 1\\ 0,& x>1\end{cases} }
{ -6q(1-x^2)^2 }
{ -6(1-x^2)(1-5q^2) }
{ \tfrac{16}{315} }
...
{ \begin{cases} \tfrac{1+\cos(\pi x)}{2}, & 0\le x\le 1\\ 0,& x>1\end{cases} }
{ -\tfrac{\pi}{2}\sin(\pi x) }
...
% \end{align}
% where $V_{3D}$ is the ball of radius $\Radius$ centered at $c$.

% First, let's exploit the radial symmetry of the metaball and rewrite $g(\p) = 1 - r$ by using the spherical coordinates of the normalized point $(\p - c)/\Radius$, where $r \in [0,1]$.
...
% The linear falloff used above, $g(\p)=\max\!\left(1-\frac{\abs{\p - c}}{\Radius},0\right)$ is only one choice, we thought to be the simplest. More generally, let
% \begin{align}
%     x=\frac{\abs{\p - c}}{\Radius},\qquad
%     g(\p)=\phi(x),
% \end{align}
