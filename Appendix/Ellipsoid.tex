...
\subsection*{Strategy C: cut by plane (solve for $z_0$)}
...
% Let the global position be the column vector $ \vec{r} = [ x,\;y,\;z ]^\transpose \in\mathbb R^{3}$. An ellipsoid of semi-axes $a,b,c>0$, expressed in its local (unrotated) frame, satisfies the implicit equation
...
%     A = \operatorname{diag}\!\left(1/a^{2}, 1/b^{2}, 1/c^{2}\right),
% \end{equation}
% where $\vec{r'} = [x',y',z']^\transpose $ are local coordinates.

% Let $R$ be the rotation matrix that takes global coordinates into the ellipsoid frame; hence

% \begin{equation}
%     \label{eq:app-rprime}
%     \vec{r'} \;=\; R^\transpose   \vec{r}.
% \end{equation}
% Substituting~\ref{eq:app-rprime} into \ref{eq:app-canonical-ellipsoid} and defining $Q \;=\; R A R^\transpose $, we obtain the ellipsoid in global coordinates:

% \begin{equation}
%     \label{eq:app-global-implicit}
%      \vec{r}^\transpose  Q  \vec{r} \;=\; 1.
% \end{equation}
% Because $A$ is diagonal and $R$ orthogonal, $Q$ is symmetric ($Q^\transpose =Q$).  Write
...
%     \left[ -\partial_x H,\; -\partial_y H,\; 1 \right]^\transpose .
% \end{equation}
...
%     \quad
%     \vec e_x' = 
%     \operatorname{normalize}\!\left[ 1,0,-\partial_x H \right]^\transpose ,
%     \quad
%     \vec e_y' = \vec e_z' \times \vec e_x'.
% \end{equation}
...
% \begin{align}
%     \vec n^\transpose ( \vec{r}- \vec{r}_0)=0,
%     \quad
%      \vec{r}_0 = 
%     \begin{bmatrix} 
%         x_0 \\ 
...
% \begin{align}
%      \vec{r}^\transpose Q \vec{r} \le 1,
%     \qquad z' = \left(R^\transpose  \vec{r}\right)_3 \;\ge\; 0.
% \end{align}
