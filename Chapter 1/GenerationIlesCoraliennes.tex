...
\comment{
    Here, need to include methods like 3D radial methods. constrained geometry \cite{Talgorn2018}.
}

\comment{
    Explain the pros/cons of those approaches: typically, they give a lot of control, but may require multiple sketches or the results depend heavily on user skill. 
    
    To date, most sketch-based terrain tools focus on either top-down features or perspective silhouettes. Combining an explicit top outline with a separate profile sketch is a new approach introduced in our work. \\
    -> clear line of novelty.
}
...
Finally, Panagiotou and Charou inverted the terrain-generation paradigm by training a cGAN to predict DEMs directly from aerial RGB imagery \cite{Panagiotou2020}. Their image-to-DEM translation showcases how GANs can handle inverse reconstruction tasks, but it hinges on large collections of paired satellite images and elevation data, which are resources that are sparse and inconsistent for many coastal and coral environments. Moreover, the inherent ambiguity of mapping color to height leads to blurred or uncertain elevation outputs when multiple plausible terrains correspond to the same visual appearance.
...
% Guérin et al.\ (2017) pioneered the use of conditional GANs for interactive sketch-to-terrain authoring \cite{Guerin2017}. They paired simple user strokes, marking ridges, valleys, peaks, and basins, with high-resolution DEM patches from NASA’s topography data, and trained a U-Net generator plus PatchGAN discriminator to translate coarse line sketches into fully detailed height fields. This approach delivers real-time feedback and preserves fine geomorphic detail, yet demands extensive, manually paired sketch–DEM examples and can falter on terrain features outside its training distribution.

% Beckham and Pal (2017) separated geometry synthesis from appearance by first training a DCGAN on bare-terrain height fields to learn a smooth latent manifold, then applying a pix2pix cGAN to overlay realistic textures conditioned on semantic masks \cite{Beckham2017}. Their two-stage pipeline enables continuous interpolation between distinct terrain types in latent space and flexible texture stylization, but introduces potential misalignment between generated geometry and subsequent texture, and remains limited by the diversity of its initial height-field training set.

% Anagnostopoulos et al.\ (2021) showed that even minimal semantic guidance (binary ridge/valley masks) can drive a pix2pix-style cGAN to synthesize plausible terrain when trained on a mixture of synthetic masks and a modest real DEM collection \cite{Anagnostopoulos2021}. By blending procedurally generated feature masks with actual elevation data, they improve generalization beyond purely real datasets. However, their model still relies on paired examples and exhibits tiling artifacts when generating regions larger than its native 256×256 patch size.

% Panagiotou and Charou (2020) inverted the typical workflow by using a cGAN to perform “image-to-DEM” translation: their generator takes high-resolution aerial RGB imagery as input and produces corresponding elevation maps \cite{Panagiotou2020}. While this demonstrates GANs’ versatility in inverse reconstruction, it depends on large corpora of accurately co-registered satellite and DEM pairs which are scarce for coral reef islands and often yields blurred elevations when multiple plausible height profiles correspond to the same surface appearance.

% These examples highlight the strengths of cGANs in capturing fine terrain statistics, fusing geometry with texture, and translating between modalities, but also their dependence on extensive paired data. In the next section, we address this data bottleneck by procedurally synthesizing our own sketch–height-field pairs for coral-reef islands, and training a pix2pix network to map user-drawn label maps directly to realistic island terrains without any real DEM examples.  
...
Once trained, the cGAN model generates a height field from a given label map. The output is a complete 2D height map representing the terrain's elevation at each point. Although the cGAN works as a \remove{black box}, the label map remains available after inference and retains valuable information for the user.
...
\add{
    A label map is a symbolic representation of the terrain, where each pixel of the map (each color) represents a class. We can use this information to keep a sparse representation of the environment, for which we will see how to treat in the next chapter about EnvObj. \\
    The training of the model can be done in the other direction, from height field to label map. \\
    If we wish to extract its content into EnvObj, we can use the label map, skeletonize it, then attribute to each resulting polyline a class, that we will call, in the next chapter, EnvObj.
}
