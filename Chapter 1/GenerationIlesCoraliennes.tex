...
\begin{Itemize}
    ...
    {In Murray's theory, with the island fixed in place but reducing in size due to erosion, reefs simply remains at sea level and grows outwards (producing fringing reefs, then barrier reefs, and eventually atolls) without any subsidence.}{fig:coral-island-murray-theory}{Murray's stand-still theory}
    ...
    {In Daly's theory, repeated sea-level drops expose the reef to wave planation and erosion into a flat bench, and subsequent high stands see new coral rims accrete on that outer terrace to form a lagoon-separated barrier reef.}{fig:coral-island-daly-theory}{Daly's glacial-control theory}
...
\begin{Itemize}
    \AltTextImage{
        ...
        \Item{} \textbf{Uniform profile shape:} Although real islands have ridges, valleys, local bumps and many other features, we simplify the model to a single elevation curve from centre to edge, making the profile-view sketch a one-dimensional drawing, enabling users to modify elevation with a single stroke.
    }
...
}{sketchingGain2009-s.png}{\cite{Gain2009} propose to edit terrains through sketching by diffusing a noisy height function over a bounded region (left), effectively modelling mountain ranges (right).}{fig:coral-island-Gain2009}{\cite{Gain2009}'s sketching method}
...
}{sketching-Hnaidi2010.png}{\cite{Hnaidi2010} defines the surface by elevation curves with constrained slopes, forming with ease river beds and cliffs.}{fig:coral-island-Hnaidi2010}{\cite{Hnaidi2010}'s sketching method}
...
}{sketchingTasse2014-s.png}{\cite{Tasse2014} use sketching in association with camera position and direction to constrain the edition of height fields.}{fig:coral-island-Tasse2014}{\cite{Tasse2014}'s sketching method}
...
}{sketching-Gasch2020-s.png}{\cite{Gasch2020} use a top-view sketch to apply constraints (a) on the noise function modelling a terrain, creating an intuitive means to create paths and roads in the resulting terrain (b and c).}{fig:coral-island-Gasch2020}{\cite{Gasch2020}'s sketching method}
...
}{sketching-Talgorn2018.png}{\cite{Talgorn2018} use top-view sketching to encode properties in the noise function used to model the terrain surface.}{fig:coral-island-Talgorn2018}{\cite{Talgorn2018}'s sketching method}
...
}{sketching-Guerin2022.png}{\cite{Guerin2022} apply gradient (left) and height (right) constraints on curves and diffuse them on the terrain to create landscapes with lightweight and multi-scale representations.}{fig:coral-island-Guerin2022}{\cite{Guerin2022}'s gradient domain terrain modelling}
...
}{sketching-Genevaux2015-1.png,sketching-Genevaux2015-2.png}{\cite{Genevaux2015} symbolically define a terrain by an aggregation of patches with real-world meaning.}{fig:coral-island-Genevaux2015}{\cite{Genevaux2015} sparse terrain representation}
...
}{sketching-Natali-2012-original.png}{\cite{Natali2012} use sketching to describe geological phenomena: (top) geological layer deformation, (middle) abrupt displacement from faulting, and (bottom) texture deformation.}{fig:coral-island-Natali2012}{\cite{Natali2012}'s geological storytelling method}
...
\begin{figure}
    ...
    \autofitgraphics[]{volcanic-render-1-labels.png, volcanic-render-1-out.png, volcanic-render-1-neutral.png, volcanic-render-1-render.png}
    \autofitgraphics[]{volcanic-render-2-labels.png, volcanic-render-2-out.png, volcanic-render-2-neutral.png, volcanic-render-2-render.png}
    \autofitgraphics[]{volcanic-render-3-labels.png, volcanic-render-3-out.png, volcanic-render-3-neutral.png, volcanic-render-3-render.png}
    \autofitgraphics[]{volcanic-render-4-labels.png, volcanic-render-4-out.png, volcanic-render-4-neutral.png, volcanic-render-4-render.png}
    \autofitgraphics[]{volcanic-render-5-labels.png, volcanic-render-5-out.png, volcanic-render-5-neutral.png, volcanic-render-5-render.png}
...
\begin{figure}
    ...
    \autofitgraphics[trim={7cm 0 7cm 0}, clip]{island-1-neutral.png, island-2-neutral.png, island-3-neutral.png, island-4-neutral.png}
    \autofitgraphics[trim={7cm 0 7cm 0}, clip]{island-1-render.png, island-2-render.png, island-3-render.png, island-4-render.png}
    % \autofitgraphics[]{island-1-volcano-out.png, island-2-volcano-out.png, island-3-volcano-out.png, island-4-volcano-out.png}
    % \autofitgraphics[trim={7cm 0 7cm 0}, clip]{island-1-volcano-neutral.png, island-2-volcano-neutral.png, island-3-volcano-neutral.png, island-4-volcano-neutral.png}
    % \autofitgraphics[trim={7cm 0 7cm 0}, clip]{island-1-volcano-render.png, island-2-volcano-render.png, island-3-volcano-render.png, island-4-volcano-render.png}
    % \autofitcaptions{Input sketches, Output height field, Neutral result, Shaded result}
...
\begin{figure}
    ...
    % \autofitgraphics[trim={7cm 0 7cm 0}, clip]{island-1-neutral.png, island-1-volcano-neutral.png, island-2-neutral.png, island-2-volcano-neutral.png, island-3-neutral.png, island-3-volcano-neutral.png, island-4-neutral.png, island-4-volcano-neutral.png}
    % \autofitgraphics[trim={7cm 0 7cm 0}, clip]{island-1-render.png, island-1-volcano-render.png, island-2-render.png, island-2-volcano-render.png, island-3-render.png, island-3-volcano-render.png, island-4-render.png, island-4-volcano-render.png}

    \autofitgraphics[]{island-1-out.png,island-1-volcano-out.png, small-white.png, island-2-out.png,island-2-volcano-out.png}
    \autofitgraphics[trim={7cm 0 7cm 0}, clip]{island-1-neutral.png, island-1-volcano-neutral.png, small-white.png, island-2-neutral.png, island-2-volcano-neutral.png}
    \autofitgraphics[trim={7cm 0 7cm 0}, clip]{island-1-render.png, island-1-volcano-render.png, small-white.png, island-2-render.png, island-2-volcano-render.png}
    ...
    \autofitgraphics[trim={7cm 0 7cm 0}, clip]{island-3-neutral.png, island-3-volcano-neutral.png, small-white.png, island-4-neutral.png, island-4-volcano-neutral.png}
    \autofitgraphics[trim={7cm 0 7cm 0}, clip]{island-3-render.png, island-3-volcano-render.png, small-white.png, island-4-render.png, island-4-volcano-render.png}
    \autofitcaptions[]{"initial", "volcanic", ~, "initial", "volcanic"}
...
Because our curve-based procedural phase included low randomness, the output of the cGAN is limiting its unpredictability, meaning that small input changes result only in minor changes on the output, preventing unexpected results. \cref{fig:coral-island-results-subsidence} shows the result of an input map with only a variation on the subsidence level, the resulting height fields are very similar. Adding the real-time computation of outputs, it becomes possible to construct progressively a landscape and correct small mistakes to intuitively design islands inspired by real-world regions. A creation of an island similar to Mayotte, presented in \cref{fig:coral-island-example-Mayotte} was hand-made in just a few minutes. The islands presented at the beginning of the chapter in \cref{fig:coral-island-island-examples} are reproduced in \cref{fig:coral-island-results-reproductions} with different subsidence rates.

\cref{fig:coral-island-results-volcanic-training} presents results of a second cGAN training for which the procedural parameters used for the generation of the training dataset. The $\heightProfile$ function include a hole at the center of the island to emulate a volcaninc crater, which we can see strongly in the first and last examples, and more slightly in the second example; the resistance function $\resistance$ is lowered on the reefs, creating visible dents in the reef and dendritic patterns on the island sides.
