...
\begin{algorithm}[H]
    ...
    $\Tilde{\warp}(\p) \gets \bigl(1-\resistance(\distRegions_\p)\bigr)\cdot \warp(\p)$ \tcp*{Modulation from resistance function}
    }
...
\begin{algorithm}[H]
    ...
    \ForEach{$\p$}{ $\hat{h}(\p) \gets \smoothmax\bigl(\heightSubsid(\p),\, c(\p)\bigr)$ }
