...
\citeName{Li2021} proposes to procedurally synthesizes coral colonies structures by emulating stochastic reef growth with a Diffusion-Limited Aggregation process, producing reef patterns but does not propose control over the simulation \cite{Li2021}. The integration of user input such as sketch interfaces remains essential for controlling the generated output.
...
\AltTextImage{
    Building on the need for more intuitive editing, \citeName{Guerin2022} introduce gradient domain paradigm for terrain modelling \cite{Guerin2022}. Rather than specifying elevation values directly, users interact with slope-based representations, allowing for the manipulation of terrain inclination and the integration of local edits into global terrain structure (\cref{fig:coral-island-Guerin2022}). By controlling terrain gradients and reconstructing elevation through iterative optimisation process, this method enables seamless blending between regions and supports a natural editing workflow, particularly for sculpting realistic mountain ridges, valleys, or plateaus.
...
Early terrain GANs used unconditional generation, synthetising elevation maps from pure latent noise, without any spatial or semantic guidance. \citeName{WulffJensen2018} train a Deep Convolutional GAN (DCGAN) to generate realistic digital elevation models of mountainous landscapes, showing that terrain-like structures could emerge from purely data-driven learning \cite{WulffJensen2018}. The model captures local elevation statistics and allowed for latent space interpolation, enabling smooth variations across generated terrains. However, the lack of spatial conditioning makes it difficult to control or constrain features such as ridges, valleys, and coastlines, resulting in landscapes that reflected training set distributions but offered no means for intentional design.
...
\citeName{Naik2022} insert a Variational Autoencoder \cite{Kingma2022} before the cGAN phase to first compress the user input in a latent space \cite{Naik2022}, allowing to forward pass the cGAN with variations of the user input sketch or interpolate between different inputs in the 128 dimensions offered by the latent space. 
...
\begin{align}
    ...
    \warp(\p) = \sum_{\curve \in \text{curves}} \warp_\curve(\p) .
\end{align}

Once the deformation vector $\warp(\p)$ is computed, the terrain height at point $\p$ is adjusted by displacing $\p$ to a new point $\p + \warp(\p)$, forming the warp map $W = \p + \warp(\p)$.
...
Our use of a scaling factor contracts the terrain toward the height field's numerical zero-elevation level. Since this zero level is merely an internal reference of the procedural model and does not correspond to a geophysical reference frame, the approach should be understood as a computational simplification rather than a physically accurate representation of tectonic subsidence.
...
We model transition zones between these regions using a smoothstep operator \cite{Perlin2002} for $x \in [0, 1]$:
\begin{align*}
    \smooth(x) = 3x^2 - 2x^3.
\end{align*}
...
In the examples below, all terrains are defined in normalised coordinates $(x, y) \in [-1, 1]^2$ and $z \in [0, 1]$. Noise amplitudes used are bounded by $\abs{\noise_\text{outlines}} \leq 0.2$, $0.8 \leq \noise_\text{height} \leq 1.2$, $0.8 \leq \noise_\text{resistance} \leq 1.2$.
...
\begin{figure}
    ...
    % \autofitcaptions{Input sketches, Output height field, Neutral result, Shaded result}
    \caption[Reproduction of islands from \cref{fig:coral-island-island-examples}]{Reproduction of islands from \cref{fig:coral-island-island-examples}, with, from top to bottom: reference island, a label map sketched, output from the "initial" cGAN model, 3D view of the output, and shaded render of the output. }
    \label{fig:coral-island-results-reproductions}
\end{figure}
...
\begin{figure}
    ...
    \autofitcaptions[]{"initial", "volcanic", ~, "initial", "volcanic"}
    \caption[Comparison of cGAN trained models]{Comparison between the "initial" cGAN trained model (dataset generated from parameters visible in \cref{fig:coral-island-wind-effect-result}) and the "volcanic" cGAN (\cref{fig:coral-island-results-volcanic-training}, top). The input label maps are identical as \cref{fig:coral-island-results-reproductions}. For each cGAN model and from each island reproduced, we display from top to bottom: cGAN model's output, 3D view, and a shaded render. }
