...
Using a modified version the Active Contours algorithm \cite{Kass1988}, we can optimize the energy for the curve $\curve$ given $\energy = \Einternal + \Eshape + \Eimage$ with  
\begin{align}
    \label{eq:internal-energy-equation}
    \Einternal &= \frac{1}{2} \left( \norm{\frac{d v}{d t}(t)}^2 + \norm{\frac{d^2 v}{d t^2}(t)}^2  \right) \\
    \Eshape &= \left( \Length - \int_{0}^{1}{\curve'(t) \, dt} \right)^2 \\
    \Eimage &= \int_{\p \in \curve} \fittingFunc(\p) \, d\p
\end{align}
